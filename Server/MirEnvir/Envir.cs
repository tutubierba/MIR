...
namespace Server.MirEnvir
{
    ...
    public class Envir
    {
        ...
        {
            //战士
            if (!MagicExists(Spell.Fencing)) MagicInfoList.Add(new MagicInfo { Name = "基本剑法", Spell = Spell.Fencing, Icon = 2, Level1 = 7, Level2 = 9, Level3 = 12, Need1 = 270, Need2 = 600, Need3 = 1300, Range = 0 });
        ...
        private void StartEnvir()
        {
            ...
            MessageQueue.Enqueue($"{BuffInfoList.Count} 增益已加载.");
            ...
            MessageQueue.Enqueue($"{RecipeInfoList.Count} 物品合成配方已加载.");

            for (var i = 0; i < MapInfoList.Count; i++)
            {
                MapInfoList[i].CreateMap();
            }
            MessageQueue.Enqueue($"{MapInfoList.Count} 地图信息已加载.");
        ...
        public void Login(ClientPackets.Login p, MirConnection c)
        {
            ...
            MessageQueue.Enqueue(account.Connection.SessionID + ", " + account.Connection.IPAddress + ", 用户已登陆.");
        ...
        public bool CanCreateHero(ClientPackets.NewHero p, MirConnection c, bool IsGm)
        {
            ...
            if (p.Class != MirClass.战士 && p.Class != MirClass.法师 && p.Class != MirClass.道士 && p.Class != MirClass.刺客 && p.Class != MirClass.弓箭手)
            {
                c.Enqueue(new S.NewHero { Result = 3 });
                return false;
            }

            if (p.Class == MirClass.刺客 && !Settings.AllowCreateAssassin || p.Class == MirClass.弓箭手 && !Settings.AllowCreateArcher)
            {
                c.Enqueue(new S.NewHero { Result = 3 });
                return false;
            }
