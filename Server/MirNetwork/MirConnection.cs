...
namespace Server.MirNetwork
{
    ...
    public class MirConnection
    {
        ...
        private void ProcessPacket(Packet p)
        {
            ...
            switch (p.Index)
            {
                ...
                case (short)ClientPacketIds.ConfirmItemRental:
                    ...
                    break;
                default:
                    MessageQueue.Enqueue(string.Format("收到无效数据包。索引：｛0｝", p.Index));
                    break;
            }
        }
        ...
        private void Login(C.Login p)
        {
            ...
            MessageQueue.Enqueue(SessionID + ", " + IPAddress + ", 用户登录中.");
            Envir.Login(p, this);
        }
        ...
        private void ChangeMarriage(C.ChangeMarriage p)
        {
            ...
            if (Player.Info.Married == 0)
            {
                ...
                if (Player.AllowMarriage)
                    Player.ReceiveChat("你现在允许结婚请求.", ChatType.Hint);
                else
                    Player.ReceiveChat("你阻止了结婚请求.", ChatType.Hint);
            }
            else
            {
                Player.AllowLoverRecall = !Player.AllowLoverRecall;
                if (Player.AllowLoverRecall)
                    Player.ReceiveChat("你现在允许爱人召回.", ChatType.Hint);
                else
                    Player.ReceiveChat("你阻止了爱人的召回.", ChatType.Hint);
            }
        }
