    ...
    public class QuestProgressInfo
    {
        ...
        public void UpdateKillTasks()
        {
            if(Info.KillMessage.Length > 0 && Info.KillTasks.Count > 0) 
            {
                ...
                TaskList.Add(string.Format("{0} {1}", Info.KillMessage, allComplete ? "(进度)" : ""));
                return;
            }
            ...
            for (int i = 0; i < KillTaskCount.Count; i++)
            {
                ...
                {
                    TaskList.Add(string.Format("击杀 {0}: {1}/{2} {3}", KillTaskCount[i].Info.Monster.GameName, KillTaskCount[i].Count,
                        KillTaskCount[i].Info.Count, KillTaskCount[i].Complete ? "(进度)" : ""));
                }
                else
                {
                    TaskList.Add(string.Format("{0} {1}", Info.KillTasks[i].Message, KillTaskCount[i].Complete ? "(进度3)" : ""));
                }
            }
        }
        ...
        public void UpdateItemTasks()
        {
            if (Info.ItemMessage.Length > 0 && Info.ItemTasks.Count > 0)
            {
                ...
                TaskList.Add(string.Format("{0} {1}", Info.ItemMessage, allComplete ? "(进度4)" : ""));
                return;
            }
            ...
            for (int i = 0; i < ItemTaskCount.Count; i++)
            {
                if (string.IsNullOrEmpty(Info.ItemTasks[i].Message))
                {
                    TaskList.Add(string.Format("收集{0}: {1}/{2} {3}", Info.ItemTasks[i].Item.FriendlyName, ItemTaskCount[i].Count,
                        Info.ItemTasks[i].Count, ItemTaskCount[i].Complete ? "(进度5)" : ""));
                }
                else
                {
                    TaskList.Add(string.Format("{0} {1}", Info.ItemTasks[i].Message, ItemTaskCount[i].Complete ? "(进度6)" : ""));
                }
            }
        }
        ...
        public void UpdateFlagTasks()
        {
            if (Info.FlagMessage.Length > 0)
            {
                ...
                TaskList.Add(string.Format("{0} {1}", Info.FlagMessage, allComplete ? "(进度7)" : ""));
                return;
            }
            ...
            for (int i = 0; i < FlagTaskSet.Count; i++)
            {
                ...
                {
                    TaskList.Add(string.Format("激活标志 {0} {1}", Info.FlagTasks[i].Number, FlagTaskSet[i].Complete ? "(进度8)" : ""));
                }
                else
                {
                    TaskList.Add(string.Format("{0} {1}", Info.FlagTasks[i].Message, FlagTaskSet[i].Complete ? "(进度9)" : ""));
                }
            }
        }
