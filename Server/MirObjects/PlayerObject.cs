...
namespace Server.MirObjects
{
    public class PlayerObject : HumanObject
    {
        ...
        protected override void Load(CharacterInfo info, MirConnection connection)
        {
            if (info.Player != null)
            {
                throw new InvalidOperationException("用户信息不能为空.");
            }
            ...
            {
                IsGM = true;
                MessageQueue.Enqueue(string.Format("{0}已被设置为GM", Name));
            }
        ...
        private string LogOutReason(byte reason)
        {
            switch (reason)
            {
                ...
                case 1:
                    return string.Format("{0}已退出游戏 原因：重复登录", Name);
                case 2:
                    return string.Format("{0}已退出游戏 原因：聊天信息过长", Name);
                case 3:
                    return string.Format("{0}已退出游戏 原因：服务器崩溃", Name);
                case 4:
                    return string.Format("{0}已退出游戏 原因：被管理员踢下线", Name);
                case 5:
                    return string.Format("{0}已退出游戏 原因：已达最大连接数", Name);
                case 10:
                    return string.Format("{0}已退出游戏 原因：客户端版本错误", Name);
                case 20:
                    return string.Format("{0}已退出游戏 原因：断开连接", Name);
                case 21:
                    return string.Format("{0}已退出游戏 原因：连接超时", Name);
                case 22:
                    return string.Format("{0}已退出游戏 原因：退出游戏", Name);
                case 23:
                    return string.Format("{0}已退出游戏 原因：返回角色选择", Name);
                case 24:
                    return string.Format("{0}已退出游戏 原因：观察模式", Name);
                default:
        ...
        public override void Process()
        {
            ...
            if (NewMail)
            {
                ReceiveChat("新邮件", ChatType.System);
            ...
            if (Account.HasExpandedStorage && Envir.Now > Account.ExpandedStorageExpiryDate)
            {
                ...
                ReceiveChat("扩展背包已过期.", ChatType.System);
        ...
        public override void Die()
        {
            if (SpecialMode.HasFlag(SpecialItemMode.Revival) && Envir.Time > LastRevivalTime)
            {
                ...
                for (var i = (int)EquipmentSlot.RingL; i <= (int)EquipmentSlot.RingR; i++)
                {
                    ...
                    RefreshStats();
                    ReceiveChat("你获得了重生", ChatType.System);
                    return;
                }
            }
            ...
            if (LastHitter != null && LastHitter.Race == ObjectType.Player)
            {
                ...
                {
                    hitter.ReceiveChat(string.Format("你受到正当防卫规则的保护"), ChatType.System);
                }
            ...
            if (killer == null || killer.Race != ObjectType.Player)
            {
                for (var i = 0; i < Info.Equipment.Length; i++)
                {
                    ...
                    if (item.Info.Bind.HasFlag(BindMode.BreakOnDeath))
                    {
                        ...
                        ReceiveChat($"你的{item.FriendlyName}由于死亡而破碎.", ChatType.System2);
                        Report.ItemChanged(item, item.Count, 1, "死亡掉落");
                    }
                    ...
                    if (item.Count > 1)
                    {
                        ...
                        Report.ItemChanged(item, count, 1, "死亡掉落");
                    }
                    else if (Envir.Random.Next(10) == 0)
                    {
                        if (Envir.ReturnRentalItem(item, item.RentalInformation?.OwnerName, Info))
                        {
                            ...
                            ReceiveChat($"你死亡了，{item.Info.FriendlyName} 已物归原主.", ChatType.Hint);
        ...
        private void StartGameSuccess()
        {
            ...
            ReceiveChat(string.Format("欢迎来到卓越传奇，祝您玩的开心~！！！"), ChatType.Hint);

            if (Settings.TestServer)
            {
                ReceiveChat("目前处于测试模式.", ChatType.Hint);
                Chat("@GAMEMASTER");
            }
            ...
            if (Info.GuildIndex != -1)
            {
                ...
                {
                    Info.GuildIndex = -1;
                    ReceiveChat("你已被移除出行会.", ChatType.System);
                }
                else
                {
            ...
            {
                ReceiveChat("你的邮箱满了.", ChatType.System);
            }
        ...
        public void Chat(string message, List<ChatItem> linkedItems = null)
        {
            ...
            if (GMLogin)
            {
                if (message == GMPassword)
                {
                    IsGM = true;
                    UpdateGMBuff();
                    MessageQueue.Enqueue(string.Format("{0}现在是GM", Name));
                    ReceiveChat("你已经是GM了", ChatType.System);
                    Envir.RemoveRank(Info);//remove gm chars from ranking to avoid causing bugs in rank list
                }
                else
                {
                    MessageQueue.Enqueue(string.Format("{0}GM登录", Name));
            ...
            if (Info.ChatBanned)
            {
                ...
                {
                    ReceiveChat("你已被禁言.", ChatType.System);
                    return;
                }
            ...
            else
            {
                if (ChatTime > Envir.Time)
                {
                    if (ChatTick >= 5 & !IsGM)
                    {
                        ...
                        ReceiveChat("禁止刷屏，你已被禁言5分钟.", ChatType.System);
                        return;
                    }
            ...
            if (message.StartsWith("/"))
            {
                ...
                {
                    ReceiveChat("拒绝接收你的消息.", ChatType.System);
                    return;
                }

                if (Info.Friends.Any(e => e.Info == player.Info && e.Blocked))
                {
                    ReceiveChat("无法给在黑名单中的玩家发送消息.", ChatType.System);
                    return;
                }
            ...
            else if (message.StartsWith("!#"))
            {
                ...
                if (player == null)
                {
                    ReceiveChat(string.Format("未找到玩家{0} .", Mentor.Name), ChatType.System);
                    return;
                }
            ...
            else if (message.StartsWith("!"))
            {
                ...
                {
                    ReceiveChat(string.Format("喊话冷却时间剩余：{0}秒", Math.Ceiling((ShoutTime - Envir.Time) / 1000D)), ChatType.System);
                    return;
                }
                if (Level < 8 && (!HasMapShout && !HasServerShout))
                {
                    ReceiveChat("8级之后才能发送喊话信息.", ChatType.System);
                    return;
                }
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    ...
                    case "CHANGEGENDER":
                        ...
                        ReceiveChat(string.Format("玩家{0}已更改为{1}", data.Name, data.Gender), ChatType.System);
                        MessageQueue.Enqueue(string.Format("玩家{0}已更改为{1}至{2}", data.Name, data.Gender, Name));
                    ...
                    case "LEVEL":
                        ...
                        else
                        {
                            ...
                            if (ushort.TryParse(parts[1], out level))
                            {
                                ...
                                ReceiveChat(string.Format("{0} {1} -> {2}.", "恭喜你升级了！", old, Level), ChatType.System);
                                MessageQueue.Enqueue(string.Format("Player {0} has been Leveled {1} -> {2} by {3}", Name, old, Level, Name));
                                return;
                            }
                        }

                        ReceiveChat("无法提升等级", ChatType.System);
                        break;
                    ...
                    case "MAKE":
                        {
                            ...
                            MessageQueue.Enqueue(string.Format("{0}制造了{1}X{2}", Name, iInfo.Name, tempCount));
                        }
                        break;
                    case "CLEARBUFFS":
                    ...
                    case "WUDI":
                        ...
                        hintstring = GMNeverDie ? "无敌模式" : "退出无敌模式.";
                    ...
                    case "GM":
                        ...
                        hintstring = GMGameMaster ? "管理模式" : "退出管理模式.";
                        ReceiveChat(hintstring, ChatType.Hint);
                        UpdateGMBuff();
                        break;
                    ...
                    case "OBSERVER":
                        ...
                        hintstring = Observer ? "观察模式" : "退出观察模式.";
                        ReceiveChat(hintstring, ChatType.Hint);
                        UpdateGMBuff();
                        break;
                    case "ALLOWGUILD":
                        EnableGuildInvite = !EnableGuildInvite;
                        hintstring = EnableGuildInvite ? "允许加入行会" : "禁止加入行会.";
                        ReceiveChat(hintstring, ChatType.Hint);
                        break;
                    case "RECALL":
                        if (!IsGM) return;
                    ...
                    case "天地合一":
                        ...
                        hintstring = EnableGroupRecall ? "开启天地合一" : "关闭天地合一.";
                        ReceiveChat(hintstring, ChatType.Hint);
                        break;
                    ...
                    case "GROUPRECALL":
                        ...
                        {
                            ReceiveChat("此地图禁止传唤", ChatType.System);
                            return;
                        }

                        if (Envir.Time < LastRecallTime)
                        {
                            ReceiveChat(string.Format("传唤冷却时间还有：{0}秒", (LastRecallTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                        ...
                        if (ItemSets.Any(set => set.Set == ItemSet.Recall && set.SetComplete))
                        {
                            ...
                            for (var i = 1; i < GroupMembers.Count(); i++)
                            {
                                if (GroupMembers[i].EnableGroupRecall)
                                    ...
                                    GroupMembers[i].ReceiveChat("未开启天地合一，传唤失败，请输入[@天地合一]命令开启天地合一",
                                        ChatType.System);
                            }
                        }
                        break;
                    case "RECALLMEMBER":
                        ...
                        if (Dead)
                        {
                            ReceiveChat("死亡状态无法传唤.", ChatType.System);
                            return;
                        }
                        ...
                        if (Envir.Time < LastRecallTime)
                        {
                            ReceiveChat(string.Format("传唤剩余冷却时间：{0}秒", (LastRecallTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                        if (ItemSets.Any(set => set.Set == ItemSet.Recall && set.SetComplete))
                        {
                            ...
                            {
                                player.ReceiveChat("未开启天地合一，传唤失败，请输入[@天地合一]命令开启天地合一",
                                        ChatType.System);
                                ReceiveChat((string.Format("{0}未开启天地合一，传唤失败", player.Name)), ChatType.System);
                                return;
                            }
                        ...
                        {
                            ReceiveChat("记忆套装未穿戴齐全，无法使用天地合一功能.", ChatType.System);
                            return;
                        }
                        break;
                    ...
                    case "RECALLLOVER":
                        ...
                        if (Dead)
                        {
                            ReceiveChat("死亡状态下无法传唤.", ChatType.System);
                            return;
                        }
                        ...
                        {
                            ReceiveChat("你需要戴结婚戒指才能召回.", ChatType.System);
                            return;
                        }
                        ...
                        if (Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing == Info.Married)
                        {
                            ...
                            {
                                ReceiveChat($"禁止通过结婚戒指进行远程传送.", ChatType.System);
                                return;
                            }

                            if (player == null)
                            {
                                ReceiveChat((string.Format("玩家{0}未在线.", Lover.Name)), ChatType.System);
                                return;
                            }

                            if (player.Dead)
                            {
                                ReceiveChat("你无法召唤死亡的玩家.", ChatType.System);
                                return;
                            }

                            if (player.Info.Equipment[(int)EquipmentSlot.RingL] == null)
                            {
                                player.ReceiveChat((string.Format("你需要佩戴结婚戒指才可以召回.", Lover.Name)), ChatType.System);
                            ...
                            {
                                player.ReceiveChat("禁止传送",
                                        ChatType.System);
                                ReceiveChat((string.Format("爱人{0}无法召回.", player.Name)), ChatType.System);
                                return;
                            }

                            if ((Envir.Time < LastRecallTime) && (Envir.Time < player.LastRecallTime))
                            {
                                ReceiveChat(string.Format("{0}秒，尝试召唤", (LastRecallTime - Envir.Time) / 1000), ChatType.System);
                                return;
                            }
                        ...
                        {
                            ReceiveChat("请佩戴结婚戒指召唤你的爱人", ChatType.System);
                            return;
                        }
                        break;
                    case "TIME":
                        ReceiveChat(string.Format("现在的时间是:{0}", Envir.Now.ToString("hh:mm tt")), ChatType.System);
                        break;
                    ...
                    case "ROLL":
                        ...
                        for (int i = 0; i < GroupMembers.Count; i++)
                        {
                            ...
                            playerSend.ReceiveChat(string.Format("{0} 移动到 {1}", Name, diceNum), ChatType.Group);
                        }
                        break;
                    ...
                    case "MAP":
                        ...
                        ReceiveChat((string.Format("您当前在{0}中，地图ID:{0}", mapTitle, mapName)), ChatType.System);
                        break;
                    ...
                    case "BACKUPPLAYER":
                        {
                            ...
                            ReceiveChat(string.Format("玩家｛0｝已备份", info.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format("玩家{0}已由{0}备份", info.Name, Name));
                        }
                        break;
                    ...
                    case "ARCHIVEPLAYER":
                        {
                            ...
                            if (data == Info)
                            {
                                ReceiveChat("无法存档您所在的玩家", ChatType.System);
                                return;
                            }
                            ...
                            if (account == null)
                            {
                                ReceiveChat(string.Format("在任何帐户中均未找到玩家{0}", parts[1]), ChatType.System);
                                return;
                            }
                            ...
                            ReceiveChat(string.Format("玩家{0}已存档", data.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format("玩家{0}已由{0}存档", data.Name, Name));
                        }
                        break;
                    ...
                    case "LOADPLAYER":
                        {
                            ...
                            if (bak == null)
                            {
                                ReceiveChat(string.Format("无法加载玩家{0}。尝试指定完整的存档文件名", parts[1]), ChatType.System);
                                return;
                            }
                            ...
                            {
                                ReceiveChat("由于ID不匹配，无法加载此玩家", ChatType.System);
                                return;
                            }

                            info = bak;

                            ReceiveChat(string.Format("玩家{0}已加载", info.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format("玩家{0}已由{1}加载", info.Name, Name));
                        }
                        break;
                    ...
                    case "RESTOREPLAYER":
                        {
                            ...
                            if (parts.Length > 2)
                            {
                                ...
                                {
                                    ReceiveChat(string.Format("找不到帐户{0}", parts[2]), ChatType.System);
                                    return;
                                }
                                ...
                                {
                                    ReceiveChat(string.Format("帐户{0}已包含{1}个字符", parts[2], Globals.MaxCharacterCount), ChatType.System);
                                    return;
                                }
                            }
                            ...
                            if (data == null)
                            {
                                if (account != null)
                                {
                                    ...
                                    if (data == null)
                                    {
                                        ReceiveChat(string.Format("无法还原玩家{0}。尝试指定完整的存档文件名", parts[1]), ChatType.System);
                                        return;
                                    }
                            ...
                            ReceiveChat(string.Format("玩家{0}已由还原", data.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format("玩家{0}已由{0}还原", data.Name, Name));
                        }
                        break;
                    ...
                    case "MOVE":
                        ...
                        {
                            ReceiveChat(("您无法在此地图上移动"), ChatType.System);
                            return;
                        }
                        if (Envir.Time < LastTeleportTime)
                        {
                            ReceiveChat(string.Format("{0}秒，尝试传送", (LastTeleportTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                    ...
                    case "MAPMOVE":
                        ...
                        if (map == null)
                        {
                            ReceiveChat((string.Format("Map {0}:[{1}]找不到", parts[1], instanceID)), ChatType.System);
                            return;
                        }
                        ...
                        switch (parts.Length)
                        {
                            case 2:
                                ReceiveChat(TeleportRandom(200, 0, map) ? (string.Format("移动到地图 {0}", map.Info.FileName)) :
                                    (string.Format("移动到地图{0}失败 ", map.Info.FileName)), ChatType.System);
                                break;
                            case 3:
                                ReceiveChat(TeleportRandom(200, 0, map) ? (string.Format("移动到地图 {0}:[{1}]", map.Info.FileName, instanceID)) :
                                    (string.Format("移动到地图失败 {0}:[{1}]", map.Info.FileName, instanceID)), ChatType.System);
                                break;
                            case 4:
                    ...
                    case "MOB":
                        ...
                        if (parts.Length < 2)
                        {
                            ReceiveChat("没有足够的参数来生成怪物", ChatType.System);
                            return;
                        }
                    ...
                    case "RECALLMOB":
                        ...
                        ReceiveChat((string.Format("宠物{0}x{1}已被召回.", mInfo2.Name, count)), ChatType.System);
                        break;
                    ...
                    case "GIVEPEARLS":
                        ...
                        if (count > 1)
                            MessageQueue.Enqueue(string.Format("玩家{0}已获得{1}颗珍珠", player.Name, count));
                        else
                            MessageQueue.Enqueue(string.Format("玩家{0}已获得{1}颗珍珠", player.Name, count));
                        break;
                    case "GIVECREDIT":
                    ...
                    case "GIVESKILL":
                        ...
                        {
                            player.ReceiveChat(string.Format("您已在级别{1}学习了{0}", skill.ToString(), spellLevel), ChatType.Hint);

                            if (player != this)
                            {
                                ReceiveChat(string.Format("{0}在级别{2}学习了{1}", player.Name, skill.ToString(), spellLevel), ChatType.Hint);
                            }
                    ...
                    case "FIND":
                        ...
                        {
                            ReceiveChat(string.Format("您无法再搜索{0}秒", (LastProbeTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                    ...
                    case "LEAVEGUILD":
                        ...
                        if(MyGuild.IsAtWar())
                        {
                            ReceiveChat("战争中不能离开行会.", ChatType.System);
                            return;
                        }
                    ...
                    case "CREATEGUILD":
                        ...
                        {
                            ReceiveChat(string.Format("玩家{0}已加入公会.", player.Name), ChatType.System);
                            return;
                        }
                        ...
                        {
                            ReceiveChat("行会名称限制为3-20个字符.", ChatType.System);
                            return;
                        }
                        ...
                        if (guild != null)
                        {
                            ReceiveChat(string.Format("行会{0}已存在.", gName), ChatType.System);
                            return;
                        }
                        ...
                        {
                            ReceiveChat(string.Format("成功创建行会 {0}", gName), ChatType.System);
                        }
                        else
                        {
                    ...
                    case "ALLOWTRADE":
                        ...
                        if (AllowTrade)
                            ReceiveChat("您现在允许交易", ChatType.System);
                        else
                            ReceiveChat("您不允许交易", ChatType.System);
                        break;
                    ...
                    case "TRIGGER":
                        ...
                        if (parts.Length >= 3)
                        {
                            ...
                            if (player == null)
                            {
                                ReceiveChat(string.Format("未找到玩家 {0}.", parts[2]), ChatType.System);
                                return;
                            }
                    ...
                    case "CHANGECLASS": //@changeclass [Player] [Class]
                        ...
                        ReceiveChat(string.Format("玩家{0}已更改为{0}", data.Name, data.Class), ChatType.System);
                        MessageQueue.Enqueue(string.Format("玩家{0}已由{2}更改为{1}", data.Name, data.Class, Name));
                    ...
                    case "STARTWAR":
                        ...
                        if (MyGuild == null)
                        {
                            ReceiveChat("你没有加入行会", ChatType.System);
                        }

                        if (MyGuild.Ranks[0] != MyGuildRank)
                        {
                            ReceiveChat("行会战需要行会掌门才能开启.", ChatType.System);
                            return;
                        }

                        if (enemyGuild == null)
                        {
                            ReceiveChat(string.Format("找不到行会{0}.", parts[1]), ChatType.System);
                            return;
                        }

                        if (MyGuild == enemyGuild)
                        {
                            ReceiveChat("不能跟自己的行会开战.", ChatType.System);
                            return;
                        }

                        if (MyGuild.WarringGuilds.Contains(enemyGuild))
                        {
                            ReceiveChat("行会战已开启.", ChatType.System);
                            return;
                        }

                        if (MyGuild.GoToWar(enemyGuild))
                        {
                            ReceiveChat(string.Format("你开启了与{0}的行会战.", parts[1]), ChatType.System);
                            enemyGuild.SendMessage(string.Format("{0}开启了行会战", MyGuild.Name), ChatType.System);
                        }
                        break;
                    case "ADDINVENTORY":
                        {
                            ...
                            if (Account.Gold >= openGold)
                            {
                                ...
                                ReceiveChat("库存增加了", ChatType.System);
                            }
                            else
                            {
                    ...
                    case "ADDSTORAGE":
                        {
                            ...
                            if (Account.Gold >= cost)
                            {
                                ...
                                if (Account.ExpandedStorageExpiryDate > Envir.Now)
                                {
                                    ...
                                    ReceiveChat("扩展仓库到期" + Account.ExpandedStorageExpiryDate.ToString(), ChatType.System);
                                }
                                else
                                {
                    ...
                    case "INFO":
                        {
                            ...
                            switch (ob.Race)
                            {
                                case ObjectType.Player:
                                    ...
                                    ReceiveChat("--人物信息--", ChatType.System2);
                                    ReceiveChat(string.Format("名字：{0}，等级：{1}，X：{2}，Y：{3}", plOb.Name, plOb.Level, plOb.CurrentLocation.X, plOb.CurrentLocation.Y), ChatType.System2);
                                    break;
                                case ObjectType.Monster:
                                    MonsterObject monOb = (MonsterObject)ob;
                                    ReceiveChat("--怪物信息--", ChatType.System2);
                                    ReceiveChat(string.Format("ID：{0}，名字：{1}", monOb.Info.Index, monOb.Name), ChatType.System2);
                                    ReceiveChat(string.Format("等级 : {0}, X : {1}, Y : {2}, Dir: {3}", monOb.Level, monOb.CurrentLocation.X, monOb.CurrentLocation.Y, monOb.Direction), ChatType.System2);
                                ...
                                case ObjectType.Merchant:
                                    ...
                                    ReceiveChat(string.Format("ID : {0}, 名字 : {1}", npcOb.Info.Index, npcOb.Name), ChatType.System2);
                    ...
                    case "TOGGLETRANSFORM":
                        if (HasBuff(BuffType.Transform, out Buff transform))
                        {
                            ...
                            hintstring = transform.Paused ? "禁用变身" : "启用变身";
                            ReceiveChat(hintstring, ChatType.Hint);
                        }                   
                        break;
                    ...
                    case "STARTCONQUEST":
                        {
                            ...
                            if (parts.Length < 1)
                            {
                                ReceiveChat(string.Format("[ConquestID]攻城战开始"), ChatType.System);
                                return;
                            }

                            if (MyGuild == null)
                            {
                                ReceiveChat(string.Format("你需要先建立一个行会才能攻城"), ChatType.System);
                                return;
                            }
                    ...
                    case "RESETCONQUEST":
                        {
                            ...
                            {
                                ReceiveChat("未找到征服或战争正在进行.", ChatType.System);
                            }
                        }
                        break;
                    case "GATES":
                        ...
                        {
                            ReceiveChat(string.Format("你目前无法控制任何大门."), ChatType.System);
                            return;
                        }
                        ...
                        if (openClose)
                        {
                            ReceiveChat(string.Format("{0}的大门已关闭.", MyGuild.Conquest.Info.Name), ChatType.System);
                        }
                        else
                        {
                            ReceiveChat(string.Format("{0}的大门已打开.", MyGuild.Conquest.Info.Name), ChatType.System);
                        }
                        break;
                    ...
                    case "CHANGEFLAG":
                        if (MyGuild == null || MyGuild.Conquest == null || !MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeRank) || MyGuild.Conquest.WarIsOn)
                        {
                            ReceiveChat(string.Format("您目前无权更改任何标志."), ChatType.System);
                            return;
                        }
                    ...
                    case "DELETESKILL":
                        ...
                        if (removed)
                        {
                            ReceiveChat(string.Format("您已删除技能 {0} 来自玩家 {1}", skill1.ToString(), player.Name), ChatType.Hint);
                            player.ReceiveChat(string.Format("{0}已从您中删除.", skill1), ChatType.Hint);
                        }
                        else
                        {
                            ReceiveChat(string.Format("无法删除技能，找不到技能"), ChatType.Hint);
                        }
        ...
        public void MoveItem(MirGridType grid, int from, int to)
        {
            ...
            if (from >= 0 && to >= 0 && from < array.Length && to < array.Length)
            {
                if (array[from] == null)
                {
                    ...
                    ReceiveChat("物品移动错误-请报告您尝试移动的物品和时间", ChatType.System);
                    Enqueue(p);
                    return;
                }
        ...
        public void TakeBackItem(int from, int to)
        {
            ...
            {
                ReceiveChat("太重了，回不来.", ChatType.System);
                Enqueue(p);
                return;
            }
        ...
        public void TakeBackHeroItem(int from, int to)
        {
            ...
            if (temp.Weight + CurrentBagWeight > Stats[Stat.BagWeight])
            {
                ReceiveChat("太重了，无法转移.", ChatType.System);
                Enqueue(p);
                return;
            }
        ...
        public override void UseItem(ulong id)
        {
            ...
            switch (item.Info.Type)
            {
                case ItemType.Potion:
                    switch (item.Info.Shape)
                    {
                        ...
                        case 2: //MysteryWater
                            if (UnlockCurse)
                            {
                                ReceiveChat("你已经可以解除装备被诅咒的物品了.", ChatType.Hint);
                                Enqueue(p);
                                return;
                            }
                            ReceiveChat("你现在可以解除装备被诅咒的物品.", ChatType.Hint);
                            UnlockCurse = true;
                            break;
                        case 3: //Buff
                            {
                                ...
                                if (item.GetTotal(Stat.MaxSC) > 0)
                                    AddBuff(BuffType.道士, this, time * Settings.Minute, new Stats { [Stat.MaxSC] = item.GetTotal(Stat.MaxSC) });
                ...
                case ItemType.Scroll:
                    ...
                    switch (item.Info.Shape)
                    {
                        ...
                        case 4: //RepairOil
                            ...
                            ReceiveChat("您的武器已部分修复", ChatType.Hint);
                        ...
                        case 7: //CreditScroll
                            if (item.Info.Price > 0)
                            {
                                ...
                                ReceiveChat(String.Format("{0} 积分已添加到您的帐户", item.Info.Price), ChatType.Hint);
                            }
                            break;
                        case 8: //MapShoutScroll
                            HasMapShout = true;
                            ReceiveChat("您已在当前地图上呐喊一声", ChatType.Hint);
                            break;
                        case 9://ServerShoutScroll
                            HasServerShout = true;
                            ReceiveChat("获得一次免费呐喊", ChatType.Hint);
                            break;
                ...
                case ItemType.Food:
                    ...
                    ReceiveChat("你的坐骑已经喂饱了.", ChatType.Hint);
                    Enqueue(new S.ItemRepaired { UniqueID = temp.UniqueID, MaxDura = temp.MaxDura, CurrentDura = temp.CurrentDura });
                ...
                case ItemType.Pets:
                    ...
                    else
                    {
                        ...
                        {
                            ReceiveChat("你已经拥有了这个坐骑.", ChatType.Hint);
                            petInfo = null;
                        }
                        ...
                        ReceiveChat("获得一个新坐骑 {" + petInfo.CustomName + "}.", ChatType.Hint);
                ...
                case ItemType.MonsterSpawn:
                    ...
                    if (conquestOnly)
                    {
                        ...
                        if (con == null)
                        {
                            ReceiveChat(string.Format("{0} 只能在征服期间繁殖.", monsterInfo.GameName), ChatType.Hint);
                            Enqueue(p);
                            return;
                        }
                    }
        ...
        public void CombineItem(MirGridType grid, ulong fromID, ulong toID)
        {
            ...
            switch (tempFrom.Info.Shape)
            {
                ...
                case 7: //slots
                    ...
                    {
                        ReceiveChat("无效的组合.", ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    if (tempTo.Info.RandomStats == null)
                    {
                        ReceiveChat("物品已具有最大持久.", ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                ...
                case 8: //Seal
                    ...
                    {
                        ReceiveChat("物料已封印.", ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    if (tempTo.SealedInfo != null && tempTo.SealedInfo.NextSealDate > Envir.Now)
                    {
                        ...
                        ReceiveChat($"无法为其他物品重新封印 {Functions.PrintTimeSpanFromSeconds(remainingSeconds, false)}.", ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                ...
                case 4: //orbs
                    ...
                    {
                        ReceiveChat("物品已达到最大添加状态.", ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    ...
                    if (!succeeded)
                    {
                        if ((tempFrom.Info.Shape == 3) && (Envir.Random.Next(15) < 3))
                        {
                            ...
                            Report.ItemChanged(array[indexTo], 1, 1, "组合物品已销毁");
            ...
            if (canSlotUpgrade && array[indexTo] != null)
            {
                ...
                ReceiveChat("物品增加了持久.", ChatType.Hint);
            ...
            if (canSeal && array[indexTo] != null)
            {
                ...
                ReceiveChat($"物品封印用于 {Functions.PrintTimeSpanFromSeconds(minutes * 60)}.", ChatType.Hint);
        ...
        public void PickUp()
        {
                ...
                if (item.Item != null)
                {
                    ...
                    if (item.Item.Info.ShowGroupPickup && IsGroupMember(this))
                        for (int j = 0; j < GroupMembers.Count; j++)
                            GroupMembers[j].ReceiveChat(Name + " 已拾取: {" + item.Item.FriendlyName + "}",
                                ChatType.System);
            ...
            if (sendFail)
                ReceiveChat("无法拾取，您没有此物品.", ChatType.System);
        ...
        public bool CanGainQuestItem(UserItem item)
        {
            ...
            ReceiveChat("你不能再携带任务物品了.", ChatType.System);
        ...
        public void SellItem(ulong uniqueID, ushort count)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                {
                    ReceiveChat("你不能出售这个物品.", ChatType.System);
                    Enqueue(p);
                    return;
                }
        ...
        public void RepairItem(ulong uniqueID, bool special = false)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                {
                    ReceiveChat("你不能修复这个物品.", ChatType.System);
                    return;
                }
                ...
                if (script.Types.Count != 0 && !script.Types.Contains(temp.Info.Type))
                {
                    ReceiveChat("你不能在这里修理这个物品.", ChatType.System);
                    return;
                }
        ...
        public void MarketBuy(ulong auctionID, uint bidPrice = 0)
        {
            ...
            if (MarketPanelType == MarketPanelType.GameShop)
            {
                foreach (AuctionInfo auction in Search)
                {
                    ...
                    GainItem(item);
                    Enqueue(new S.MarketSuccess { Message = string.Format("你买了{0}花费{1:#,##0}积分", auction.Item.FriendlyName, auction.Price) });
                ...
                for (int n = 0; n < CurrentMap.NPCs.Count; n++)
                {
                    ...
                    foreach (AuctionInfo auction in Search)
                    {
                        ...
                        if (auction.ItemType == MarketItemType.Consign)
                        {
                            ...
                            Envir.MessageAccount(auction.SellerInfo.AccountInfo, string.Format("你卖给{0}获得{1:#,##0}金币", auction.Item.FriendlyName, auction.Price), ChatType.Hint);
                            Enqueue(new S.MarketSuccess { Message = string.Format("你买了{0}花费{1:#,##0}金币", auction.Item.FriendlyName, auction.Price) });
                            MarketSearch(MatchName, MatchType);
                        }
                        else
                        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                foreach (AuctionInfo auction in Account.Auctions)
                {
                    ...
                    if (!auction.Sold || auction.Expired)
                    {
                        ...
                        if (auction.CurrentBuyerInfo != null)
                        {
                            string message = string.Format("你一直在超越{0}.退回{1:#,##0}金币.", auction.Item.FriendlyName, auction.CurrentBid);
                    ...
                    Envir.Auctions.Remove(auction);
                    GainGold(gold);
                    Enqueue(new S.MarketSuccess { Message = string.Format("你卖给{0}获得{1:#,##0}金币. \n收益: {2:#,##0}金币.\n佣金: {3:#,##0}金币.‎", auction.Item.FriendlyName, cost, gold, cost - gold) });
                    MarketSearch(MatchName, MatchType);
                    return;
                }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (item != null)
                {
                    if (item.UniqueID == UniqueID)
                    {
                        ...
                        {
                            ReceiveChat($"无法降级{item.FriendlyName}因为它属于{item.RentalInformation.OwnerName}", ChatType.System);
                            return;
                        }
                        ...
                        if (Info.AccountInfo.Gold >= item.DowngradePrice())
                        {
                            ...
                            {
                                case 0:
                                    ReceiveChat(string.Format("{0}:移除失败等级为0", item.FriendlyName), ChatType.System);
                                    break;
                                case 1:
                                    ...
                                    ReceiveChat(string.Format("{0}:移除成功. 等级{1}", item.FriendlyName, item.Awake.GetAwakeLevel()), ChatType.System);
                                    Enqueue(new S.RefreshItem { Item = item });
                                    break;
                                default:
                                    break;
        ...
        public void DisassembleItem(ulong UniqueID)
        {
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                {
                    ReceiveChat($"无法拆除{item.FriendlyName}", ChatType.System);
                    return;
                }

                if (item.RentalInformation != null && item.RentalInformation.BindingFlags.HasFlag(BindMode.UnableToDisassemble))
                {
                    ReceiveChat($"无法拆除{item.FriendlyName}因为它属于{item.RentalInformation.OwnerName}", ChatType.System);
                    return;
                }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (item != null)
                {
                    if (item.UniqueID == UniqueID)
                    {
                        if (item.RentalInformation != null)
                        {
                            ReceiveChat($"无法合成{item.FriendlyName}因为它属于{item.RentalInformation.OwnerName}", ChatType.System);
                            return;
                        }
        ...
        public void AddMember(string name)
        {
            ...
            {
                ReceiveChat("你的队伍已经满员了.", ChatType.System);
                return;
            }
            ...
            if (player == null)
            {
                ReceiveChat(name + "没有找到.", ChatType.System);
                return;
            }
            if (player == this)
            {
                ReceiveChat("你不能组织自己.", ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(name + "已经在另一组队伍.", ChatType.System);
                return;
            }

            if (player.GroupInvitation != null)
            {
                ReceiveChat(name + "已经收到其他组队邀请.", ChatType.System);
                return;
            }
        ...
        public void DelMember(string name)
        {
            if (GroupMembers == null)
            {
                ReceiveChat("你不是在一组.", ChatType.System);
                return;
            }
            if (GroupMembers[0] != this)
            {
                ReceiveChat("不是组长.", ChatType.System);
                return;
            }
            ...
            if (player == null)
            {
                ReceiveChat(name + "不在你的队伍.", ChatType.System);
                return;
            }
        ...
        public void GroupInvite(bool accept)
        {
            ...
            {
                ReceiveChat("你没有被邀请参加队伍.", ChatType.System);
                return;
            }

            if (!accept)
            {
                GroupInvitation.ReceiveChat(Name + "拒绝你的组队邀请.", ChatType.System);
                GroupInvitation = null;
                return;
            }
            ...
            if (GroupMembers != null)
            {
                ReceiveChat(string.Format("你不能再加入{0}的组", GroupInvitation.Name), ChatType.System);
                GroupInvitation = null;
                return;
            }
            ...
            {
                ReceiveChat(GroupInvitation.Name + "'队伍已经满员.", ChatType.System);
                GroupInvitation = null;
                return;
            }
            if (!GroupInvitation.AllowGroup)
            {
                ReceiveChat(GroupInvitation.Name + " 不允许组对.", ChatType.System);
                GroupInvitation = null;
                return;
            }
            if (GroupInvitation.Node == null)
            {
                ReceiveChat(GroupInvitation.Name + "不在线.", ChatType.System);
                GroupInvitation = null;
                return;
            }
        ...
        public bool CreateGuild(string guildName)
        {
            ...
            {
                ReceiveChat(String.Format("你的等级不足以创建一个行会会, 需要:{0}", Settings.Guild_RequiredLevel), ChatType.System);
                return false;
            }
            ...
            for (int i = 0; i < Settings.Guild_CreationCostList.Count; i++)
            {
                ...
                if (Required.Item == null)
                {
                    ...
                    {
                        ReceiveChat(String.Format("金币不足. 创建一个行会会需要{0}金币.", Required.Amount), ChatType.System);
                        return false;
                    }
                }
                else
                {
                    ...
                    if (count != 0)
                    {
                        if (Required.Amount == 1)
                            ReceiveChat(String.Format("{0}需要创建一个行会吗.", Required.Item.FriendlyName), ChatType.System);
                        else
                        {
                            if (Required.Item.Type == ItemType.Ore)
                                ReceiveChat(string.Format("{0}带有纯度{1}才能创建公会.", Required.Item.FriendlyName, Required.Amount / 1000), ChatType.System);
                            else
                                ReceiveChat(string.Format("缺少{0},你需要{1}创建公会.", Required.Item.FriendlyName, Required.Amount), ChatType.System);
                        }
                        return false;
                    }
                }
        ...
        public void EditGuildMember(string Name, string RankName, byte RankIndex, byte ChangeType)
        {
            ...
            {
                ReceiveChat("你不是在一个公会!", ChatType.System);
                return;
            }
            switch (ChangeType)
            {
                case 0: //add member
                    ...
                    {
                        ReceiveChat("不允许您招募新成员!", ChatType.System);
                        return;
                    }
                    ...
                    {
                        ReceiveChat(String.Format("{0}已加入行会!", Name), ChatType.System);
                        return;
                    }
                    if (!player.EnableGuildInvite)
                    {
                        ReceiveChat(String.Format("{0}禁用行会邀请!", Name), ChatType.System);
                        return;
                    }
                    if (player.PendingGuildInvite != null)
                    {
                        ReceiveChat(string.Format("{0}已经有一个行会邀请.", Name), ChatType.System);
                        return;
                    }
                ...
                case 2: //promote member (and it'll auto create a new rank at bottom if the index > total ranks!)
                    ...
                    {
                        ReceiveChat("你没有改变其他成员的职位的权限!", ChatType.System);
                        return;
                    }
                    if (Name == "") return;
                ...
                case 3: //change rank name
                    ...
                    {
                        ReceiveChat("职位名称不正确!", ChatType.System);
                        return;
                    }
                ...
                case 4: //new rank
                    if (!MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeRank))
                    {
                        ReceiveChat("你没有改的职位的权限!", ChatType.System);
                        return;
                    }
                    if (MyGuild.Ranks.Count > 254)
                    {
                        ReceiveChat("没有更多的职位可用.", ChatType.System);
                        return;
                    }
        ...
        public void EditGuildNotice(List<string> notice)
        {
            ...
            if (!MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeNotice))
            {
                ...
                ReceiveChat("你没有编辑行会告示的权利!", ChatType.System);
                return;
            }
            if (notice.Count > 200)
            {
                ReceiveChat("行会公告不能超过200字!", ChatType.System);
                return;
            }
        ...
        public void GuildInvite(bool accept)
        {
            ...
            {
                ReceiveChat("你还没有收到行会邀请。", ChatType.System);
                return;
            }
            if (!accept) return;
            if (!PendingGuildInvite.HasRoom())
            {
                ReceiveChat(String.Format("{0}已满员.", PendingGuildInvite.Name), ChatType.System);
                return;
            }
        ...
        public void GuildNameReturn(string Name)
        {
            ...
            if (MyGuild != null)
            {
                ReceiveChat("你已经有公会了.", ChatType.System);
                CanCreateGuild = false;
                return;
            }
        ...
        public void GuildStorageGoldChange(byte type, uint amount)
        {
            if ((MyGuild == null) || (MyGuildRank == null))
            {
                ReceiveChat("你没有行会.", ChatType.System);
                return;
            }

            if (!InSafeZone)
            {
                ReceiveChat("不能在非安全区使用行会仓库.", ChatType.System);
                return;
            }
            ...
            if (type == 0)//donate
            {
                ...
                {
                    ReceiveChat("行会金币储备已经满了.", ChatType.System);
                    return;
                }
            ...
            else
            {
                ...
                {
                    ReceiveChat("金币已经满了.", ChatType.System);
                    return;
                }
        ...
        public void GuildStorageItemChange(byte type, int from, int to)
        {
            ...
            {
                Enqueue(p);
                ReceiveChat("你不能使用行会仓库.", ChatType.System);
                return;
            }
            ...
            switch (type)
            {
                case 0://store
                    ...
                    {
                        Enqueue(p);
                        ReceiveChat("没有权限使用行会仓库.", ChatType.System);
                        return;
                    }
                    if (from < 0 || from >= Info.Inventory.Length)
                    {
                    ...
                    {
                        ReceiveChat("目标槽已占用.", ChatType.System);
                        Enqueue(p);
                        return;
                    }
                ...
                case 1://retrieve
                    if (!MyGuildRank.Options.HasFlag(GuildRankOptions.CanRetrieveItem))
                    {
                        ...
                        ReceiveChat("你没有权限从公会仓库取出物品.", ChatType.System);
                        return;
                    }
                    ...
                    {
                        ReceiveChat("超重了，无法取回物品.", ChatType.System);
                        Enqueue(p);
                        return;
                    }
                ...
                case 2: // Move Item
                    ...
                    {
                        Enqueue(p);
                        ReceiveChat("你没有权限往行会仓库存入物品.", ChatType.System);
                        return;
                    }
                    if (from < 0 || from >= MyGuild.StoredItems.Length)
                    {
        ...
        public void GuildWarReturn(string Name)
        {
            ...
            if (MyGuild.WarringGuilds.Contains(enemyGuild))
            {
                ReceiveChat("已经和这个行会开战了.", ChatType.System);
                return;
            }

            if (MyGuild.Gold < Settings.Guild_WarCost)
            {
                ReceiveChat("行会里没有足够的资金.", ChatType.System);
                return;
            }

            if (MyGuild.GoToWar(enemyGuild))
            {
                ReceiveChat(string.Format("你申请与{0}开始公会战争.", Name), ChatType.System);
                enemyGuild.SendMessage(string.Format("{0}已经与你的公会开战了", MyGuild.Name), ChatType.System);
        ...
        public void GuildBuffUpdate(byte type, int id)
        {
            ...
            switch (type)
            {
                ...
                case 1://buy the buff
                    ...
                    {
                        ReceiveChat("你的行会等级不符.", ChatType.System);
                        return;
                    }
                    GuildBuffInfo BuffInfo = Envir.FindGuildBuffInfo(id);
                    if (BuffInfo == null)
                    {
                        ReceiveChat("BUFF不存在.", ChatType.System);
                        return;
                    }
                    if (MyGuild.GetBuff(id) != null)
                    {
                        ReceiveChat("BUFF已获得.", ChatType.System);
                        return;
                    }
                ...
                case 2://activate the buff
                    ...
                    if (Buff == null)
                    {
                        ReceiveChat("BUFF没有得到.", ChatType.System);
                        return;
                    }
        ...
        public void RetrieveTradeItem(int from, int to)
        {
            ...
            if (temp.Weight + CurrentBagWeight > Stats[Stat.BagWeight])
            {
                ReceiveChat("太重了，无法回头.", ChatType.System);
                Enqueue(p);
                return;
            }
        ...
        public void TradeRequest()
        {
            ...
            if (TradePartner != null)
            {
                ReceiveChat("你已经交易.", ChatType.System);
                return;
            }
            ...
            if (player != null)
            {
                ...
                if (player == this)
                {
                    ReceiveChat("你必须面对某人才能交易.", ChatType.System);
                    return;
                }

                if (player.Dead || Dead)
                {
                    ReceiveChat("死后无法交易", ChatType.System);
                    return;
                }

                if (player.TradeInvitation != null)
                {
                    ReceiveChat(string.Format("玩家{0}已经在交易了.", player.Info.Name), ChatType.System);
                    return;
                }

                if (!player.AllowTrade)
                {
                    ReceiveChat(string.Format("玩家{0}目前不允许交易.", player.Info.Name), ChatType.System);
                    return;
                }

                if (!Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) || player.CurrentMap != CurrentMap)
                {
                    ReceiveChat(string.Format("玩家{0}距离太远不能交易.", player.Info.Name), ChatType.System);
                    return;
                }

                if (player.TradePartner != null)
                {
                    ReceiveChat(string.Format("玩家{0}正在交易中.", player.Info.Name), ChatType.System);
                    return;
                }
        ...
        public void TradeReply(bool accept)
        {
            ...
            {
                ReceiveChat(string.Format("玩家{0}正在交易中.", TradeInvitation.Info.Name), ChatType.System);
                TradeInvitation = null;
                return;
            }
        ...
        public void TradeConfirm(bool confirm)
        {
            ...
            {
                TradePartner.ReceiveChat(string.Format("玩家{0}正在等你确认交易.", Info.Name), ChatType.System);
            }
            ...
            for (int p = 0; p < 2; p++)
            {
                ...
                {
                    CanTrade = false;
                    TradePair[p].ReceiveChat("交易伙伴的背包满了.", ChatType.System);
                    TradePair[p].Enqueue(new S.TradeCancel { Unlock = true });

                    TradePair[o].ReceiveChat("交易伙伴的背包满了.", ChatType.System);
                ...
                {
                    CanTrade = false;
                    TradePair[p].ReceiveChat("交易伙伴的金币过多无法接收交易.", ChatType.System);
                    TradePair[p].Enqueue(new S.TradeCancel { Unlock = true });

                    TradePair[o].ReceiveChat("交易伙伴的金币过多无法接收交易", ChatType.System);
                    TradePair[o].Enqueue(new S.TradeCancel { Unlock = true });
            ...
            if (CanTrade)
            {
                for (int p = 0; p < 2; p++)
                {
                    ...
                    for (int i = 0; i < TradePair[p].Info.Trade.Length; i++)
                    {
                        ...
                        Report.ItemMoved(u, MirGridType.Trade, MirGridType.Inventory, i, -99, string.Format("从{0}到{1}交易", TradePair[p].Name, TradePair[o].Name));
                    }

                    if (TradePair[p].TradeGoldAmount > 0)
                    {
                        Report.GoldChanged(TradePair[p].TradeGoldAmount, true, string.Format("从{0}到{1}交易", TradePair[p].Name, TradePair[o].Name));
        ...
        public void FishingCast(bool cast, bool cancel = false)
        {
            ...
            if (cast)
            {
                ...
                if (item == null)
                {
                    ReceiveChat("你需要使用鱼钩.", ChatType.System);
                    return;
                }
            ...
            else
            {
                ...
                if (FishFound)
                {
                    ...
                    if (Envir.Random.Next(0, 100) <= getChance)
                    {
                        ...
                        if (dropItem == null)
                        {
                            ReceiveChat("你的鱼跑掉了!", ChatType.System);
                        }
                        else if (FreeSpace(Info.Inventory) < 1)
                        {
                            ReceiveChat("你的背包没有足够的空间.", ChatType.System);
                        }
                        else
                        {
                            GainItem(dropItem);
        ...
        public void AcceptQuest(int index)
        {
            ...
            {
                ReceiveChat("任务已经满了.", ChatType.System);
                return;
            }
        ...
        public void FinishQuest(int questIndex, int selectedItemIndex = -1)
        {
            ...
            {
                ReceiveChat("你的背包已满，不能接收任务奖励.", ChatType.System);
                return;
            }
        ...
        public void ShareQuest(int questIndex)
        {
            ...
            if (!shared)
            {
                ReceiveChat("不能与别人组队完成任务.", ChatType.System);
            }
        }
        ...
        public void SendMail(string name, string message)
        {
            ...
            {
                ReceiveChat("找不到玩家{0}.", ChatType.System);
                return;
            }

            if (Info.Friends.Any(e => e.Info == player && e.Blocked))
            {
                ReceiveChat("对方不接收你的邮件.", ChatType.System);
                return;
            }
        ...
        public void CollectMail(ulong mailID)
        {
            ...
            //if (!mail.Collected)
            //{
            //    ReceiveChat("邮件必须从邮件功能收集.", ChatType.System);
            //    return;
            //}
            ...
            if (mail.Items.Count > 0)
            {
                ...
                {
                    ReceiveChat("你的背包满了,无法接收物品.", ChatType.System);
                    return;
                }
        ...
        public void SummonIntelligentCreature(IntelligentCreatureType pType)
        {
            ...
            for (int i = 0; i < Info.IntelligentCreatures.Count; i++)
            {
                ...
                ReceiveChat((string.Format("宠物{0}已被召唤.", Info.IntelligentCreatures[i].CustomName)), ChatType.System);
                break;
            }
        ...
        public void UnSummonIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true)
        {
            ...
            for (int i = 0; i < Pets.Count; i++)
            {
                ...
                if (doUpdate) ReceiveChat(string.Format("宠物{0}已被召回.", pet.CustomName), ChatType.System);
        ...
        public void ReleaseIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true)
        {
            ...
            for (int i = 0; i < Info.IntelligentCreatures.Count; i++)
            {
                ...
                if (doUpdate) ReceiveChat((string.Format("宠物{0}已被释放.", Info.IntelligentCreatures[i].CustomName)), ChatType.System);
        ...
        public void RefreshCreatureSummoned()
        {
            ...
            if (!petFound)
            {
                MessageQueue.EnqueueDebugging(string.Format("{0}:召唤的宠物不再存在？!?.{1}", Name, SummonedCreatureType.ToString()));
                CreatureSummoned = false;
                SummonedCreatureType = IntelligentCreatureType.None;
            }
        }
        ...
        public bool IntelligentCreatureProduceBlackStone()
        {
            ...
            if (!CanGainItem(item, false))
            {
                MailInfo mail = new MailInfo(Info.Index)
                {
                    MailID = ++Envir.NextMailID,
                    Sender = "黑矿石",
                    Message = "您的宠物生产了x1黑石，无法添加到您的库存中.",
        ...
        public void StrongboxRewardItem(int boxtype)
        {
            ...
            if (dropItem == null)
            {
                ReceiveChat("未找到.", ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat("没有更多空间.", ChatType.System);
                return;
            }
        ...
        public void AddFriend(string name, bool blocked = false)
        {
            ...
            if (Name == name)
            {
                ReceiveChat("不能添加自己", ChatType.System);
                return;
            }

            if (Info.Friends.Any(e => e.Index == info.Index))
            {
                ReceiveChat("玩家已经添加成功", ChatType.System);
                return;
            }
        ...
        public void RetrieveRefineItem(int from, int to)
        {
            ...
            if (temp.Weight + CurrentBagWeight > Stats[Stat.BagWeight])
            {
                ReceiveChat("太重了，拿不动.", ChatType.System);
                Enqueue(p);
                return;
            }
        ...
        public void RefineItem(ulong uniqueID)
        {
            ...
            {
                ReceiveChat(String.Format("您的{0}需要检查以后再尝试进行升级.", Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }

            if ((Info.Inventory[index].Info.Type != ItemType.Weapon) && (Settings.OnlyRefineWeapon))
            {
                ReceiveChat(String.Format("你的{0}不能精炼.", Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }

            if (Info.Inventory[index].Info.Bind.HasFlag(BindMode.DontUpgrade))
            {
                ReceiveChat(String.Format("你的{0不能精炼.", Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(String.Format("你没有足够的金币来精炼你的{0}.", Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }
            ...
            if ((totalDC == 0) && (totalMC == 0) && (totalSC == 0))
            {
                ...
                {
                    ReceiveChat(String.Format("你的{0}正在精炼中,请{1}分钟后再试.", Info.CurrentRefine.FriendlyName, Settings.RefineTime), ChatType.System);
                }
        ...
        public void CollectRefine()
        {
            ...
            {
                ReceiveChat("你现在没有精炼任何物品.", ChatType.System);
                Enqueue(p);
                return;
            }
            ...
            {
                ReceiveChat(string.Format("你可以{1}分钟后来取你的 {0}.", Info.CurrentRefine.FriendlyName, ((Info.CollectTime - Envir.Time) / Settings.Minute)), ChatType.System);
                Enqueue(p);
                return;
            }
            ...
            {
                ReceiveChat(string.Format("你的{0}太重了,不可以再取回,请清理背包后在试.", Info.CurrentRefine.FriendlyName), ChatType.System);
                Enqueue(p);
                return;
            }
            ...
            if (index == -1)
            {
                ReceiveChat(String.Format("你的背包已经没有空间了,请清理背包后再取回你的 {0}.", Info.CurrentRefine.FriendlyName), ChatType.System);
                Enqueue(p);
                return;
            }

            ReceiveChat(String.Format("您的物品已退还给您."), ChatType.System);
            p.Success = true;
        ...
        public void CheckRefine(ulong uniqueID)
        {
            ...
            if (Info.Inventory[index].RefineAdded == 0)
            {
                ReceiveChat(String.Format("{0}不需要检查，因为它还没有被升级.", Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(String.Format("恭喜,你的{0} 现在 +{1} 额外攻击.", Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
            ...
            {
                ReceiveChat(String.Format("恭喜,你的{0}现在 +{1} 额外魔法.", Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
            ...
            {
                ReceiveChat(String.Format("恭喜,你的{0}现在 +{1} 额外道术.", Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
            ...
            {
                ReceiveChat(String.Format("你的{0}升级失败破碎了.", Info.Inventory[index].FriendlyName), ChatType.System);
        ...
        public void NPCDivorce()
        {
            ...
            if (player != null)
            {
                ...
                player.ReceiveChat(string.Format("你刚被强行离婚"), ChatType.System);
                if (player.Info.Equipment[(int)EquipmentSlot.RingL] != null)
        ...
        public bool CheckMakeWeddingRing()
        {
            if (Info.Married == 0)
            {
                ReceiveChat(string.Format("你要准备结婚戒指才可以结婚."), ChatType.System);
                return false;
            }

            if (Info.Equipment[(int)EquipmentSlot.RingL] == null)
            {
                ReceiveChat(string.Format("结婚戒指需要戴在左手上."), ChatType.System);
                return false;
            }

            if (Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing != -1)
            {
                ReceiveChat(string.Format("你已经戴了一个结婚戒指."), ChatType.System);
                return false;
            }

            if (Info.Equipment[(int)EquipmentSlot.RingL].Info.Bind.HasFlag(BindMode.NoWeddingRing))
            {
                ReceiveChat(string.Format("你不能使用这种类型的戒指."), ChatType.System);
                return false;
            }
        ...
        public void ReplaceWeddingRing(ulong uniqueID)
        {
            ...
            if (CurrentRing == null)
            {
                ReceiveChat(string.Format("你没有戴戒指升级."), ChatType.System);
                return;
            }

            if (CurrentRing.WeddingRing == -1)
            {
                ReceiveChat(string.Format("您没有戴结婚戒指来升级."), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(string.Format("你不能用这个来代替结婚戒指."), ChatType.System);
                return;
            }

            if (!CanEquipItem(temp, (int)EquipmentSlot.RingL))
            {
                ReceiveChat(string.Format("你不能装备这个物品."), ChatType.System);
                return;
            }
            ...
            if (cost > Account.Gold)
            {
                ReceiveChat(String.Format("你没有足够的金币来取下你的结婚戒指."), ChatType.System);
                return;
            }
        ...
        public void MarriageRequest()
        {
            ...
            {
                ReceiveChat(string.Format("你暂时不能结婚，离婚后需要{0}天才可以再婚.", Settings.MarriageCooldown), ChatType.System);
                return;
            }

            if (Info.Level < Settings.MarriageLevelRequired)
            {
                ReceiveChat(string.Format("你需要至少{0}级才能结婚..", Settings.MarriageLevelRequired), ChatType.System);
                return;
            }
            ...
            if (player != null)
            {
                ...
                if (!Functions.FacingEachOther(Direction, CurrentLocation, player.Direction, player.CurrentLocation))
                {
                    ReceiveChat(string.Format("你需要面对彼此的结婚对象."), ChatType.System);
                    return;
                }

                if (player.Level < Settings.MarriageLevelRequired)
                {
                    ReceiveChat(string.Format("{0}你的爱人需要{1}级才可以结婚.", Settings.MarriageLevelRequired), ChatType.System);
                    return;
                }

                if (player.Info.MarriedDate.AddDays(Settings.MarriageCooldown) > Envir.Now)
                {
                    ReceiveChat(string.Format("{0}你暂时不能结婚，离婚后需要{1}天才可以再婚", player.Name, Settings.MarriageCooldown), ChatType.System);
                    return;
                }

                if (!player.AllowMarriage)
                {
                    ReceiveChat("你申请的结婚对象不允许结婚.", ChatType.System);
                    return;
                }

                if (player == this)
                {
                    ReceiveChat("你不能娶你自己.", ChatType.System);
                    return;
                }

                if (player.Dead || Dead)
                {
                    ReceiveChat("不能和一个死人结婚.", ChatType.System);
                    return;
                }

                if (player.MarriageProposal != null)
                {
                    ReceiveChat(string.Format("{0}已经有一个结婚的邀请.", player.Info.Name), ChatType.System);
                    return;
                }

                if (!Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) || player.CurrentMap != CurrentMap)
                {
                    ReceiveChat(string.Format("{0}距离太远无法申请结婚.", player.Info.Name), ChatType.System);
                    return;
                }
        ...
        public void MarriageReply(bool accept)
        {
            ...
            if (!accept)
            {
                MarriageProposal.ReceiveChat(string.Format("{0}拒绝与你结婚.", Info.Name), ChatType.System);
                MarriageProposal = null;
                return;
            }
            ...
            MarriageProposal.ReceiveChat(string.Format("恭喜,你选在与{0}已经是夫妻了.", Info.Name), ChatType.System);
            ReceiveChat(String.Format("恭喜,你选在与{0}已经是夫妻了.", MarriageProposal.Info.Name), ChatType.System);
        ...
        public void DivorceRequest()
        {
            ...
            if (player == null)
            {
                ReceiveChat(string.Format("你需要面对爱人才能申请离婚."), ChatType.System);
                return;
            }
            ...
            if (player != null)
            {
                ...
                if (player.Dead || Dead)
                {
                    ReceiveChat("你不能与死人离婚.", ChatType.System); //GOT TO HERE, NEED TO KEEP WORKING ON IT.
                    return;
                }

                if (player.Info.Index != Info.Married)
                {
                    ReceiveChat(string.Format("{0}你还没结婚", player.Info.Name), ChatType.System);
                    return;
                }

                if (!Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) || player.CurrentMap != CurrentMap)
                {
                    ReceiveChat(string.Format("{0}是不是在离婚范围内.", player.Info.Name), ChatType.System);
                    return;
                }
        ...
        public void DivorceReply(bool accept)
        {
            ...
            if (!accept)
            {
                DivorceProposal.ReceiveChat(string.Format("{0}一直拒绝离婚.", Info.Name), ChatType.System);
                DivorceProposal = null;
                return;
            }
            ...
            if (Info.Married == 0)
            {
                ReceiveChat("你没有结婚，所以你不需要离婚.", ChatType.System);
                DivorceProposal = null;
                return;
            }
        ...
        public void LogoutRelationship()
        {
            ...
            if (lover == null)
            {
                MessageQueue.EnqueueDebugging(Name + " 已婚却找不到你的伴侣信息 " + Info.Married);
                return;
            }
            ...
            if (player != null)
            {
                ...
                player.ReceiveChat(String.Format("{0}下线了.", Info.Name), ChatType.System);
            }
        }
        ...
        public void MentorBreak(bool force = false)
        {
            if (Info.Mentor == 0)
            {
                ReceiveChat("你没有一个师傅取消", ChatType.System);
                return;
            }
            ...
            if (force)
            {
                ...
                ReceiveChat(String.Format("你{0}天内才可以申请新的师傅.", Settings.MentorLength), ChatType.System);
            }
            else
            {
                ReceiveChat("你的徒弟已经出师了.", ChatType.System);
            }
            ...
            if (partnerP != null)
            {
                partnerP.ReceiveChat("你的拜师已经过期.", ChatType.System);
        ...
        public void AddMentor(string Name)
        {
            if (Info.Mentor != 0)
            {
                ReceiveChat("你已经有师傅.", ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat("你不能拜一个新的师傅呢.", ChatType.System);
                return;
            }
            ...
            if (mentor == null)
            {
                ReceiveChat(String.Format("找不到{0}的名字.", Name), ChatType.System);
            }
            else
            {
                ...
                {
                    ReceiveChat(String.Format("{0}你不能拜一个新的师傅呢.", mentor.Info.Name), ChatType.System);
                    return;
                }
                ...
                {
                    ReceiveChat("你的等级已经可以出师了.", ChatType.System);
                    return;
                }
                if ((Info.Level + Settings.MentorLevelGap) > mentor.Level)
                {
                    ReceiveChat(String.Format("需要徒弟等级不大于{0}级才可以收徒.", Settings.MentorLevelGap), ChatType.System);
                    return;
                }

                mentor.MentorRequest = this;
                mentor.Enqueue(new S.MentorRequest { Name = Info.Name, Level = Info.Level });
                ReceiveChat(String.Format("发送的请求."), ChatType.System);
            }
        ...
        public void MentorReply(bool accept)
        {
            ...
            if (!accept)
            {
                MentorRequest.ReceiveChat(string.Format("{0}一直拒绝成为你的师傅.", Info.Name), ChatType.System);
                MentorRequest = null;
                return;
            }
            ...
            if (Info.Mentor != 0)
            {
                ReceiveChat("你已经有了一个徒弟了.", ChatType.System);
                return;
            }
            ...
            else
            {
                ...
                {
                    ReceiveChat(String.Format("{0}已经有一个徒弟.", student.Info.Name), ChatType.System);
                    return;
                }
                ...
                {
                    ReceiveChat(String.Format("需要达到{0}级才可以收徒.", Settings.MentorLevelGap), ChatType.System);
                    return;
                }
                ...
                ReceiveChat(String.Format("你现在是{0}的徒弟", student.Info.Name), ChatType.System);
                student.ReceiveChat(String.Format("你现在是 {0} 的师傅.", Info.Name), ChatType.System);
                GetMentor(false);
        ...
        public void LogoutMentor()
        {
            ...
            if (mentor == null)
            {
                MessageQueue.EnqueueDebugging(Name + " 已拜师却找不到你的师徒信息 " + Info.Mentor);
                return;
            }
            ...
            if (player != null)
            {
                ...
                player.ReceiveChat(String.Format("{0}已经离线了.", Info.Name), ChatType.System);
            }
        }
        ...
        public void GameshopBuy(int GIndex, byte Quantity)
        {
            ...
            if (Product == null)
            {
                ReceiveChat("你想买的商品不在店里.", ChatType.System);
                MessageQueue.EnqueueDebugging(Info.Name + " 想买一些不存在的东西.");
                return;
            }
            ...
            if (Product.Stock != 0)
            {
                ...
                {
                    ReceiveChat("你想购买更多.", ChatType.System);
                    GameShopStock(Product);
                    MessageQueue.EnqueueDebugging(Info.Name + "尝试购买" + Product.Info.FriendlyName + " x " + Quantity + " - 库存不足.");
                    return;
                }
            }
            else
            ...
            if (stockAvailable)
            {
                ...
                else
                {
                    ...
                    {
                        ReceiveChat("你没有足够的金币购买选在的商品.", ChatType.System);
                        MessageQueue.EnqueueDebugging(Info.Name + " 尝试购买 " + Product.Info.FriendlyName + " x " + Quantity + " - 没有足够的货币.");
                        return;
                    }
                }
            }
            ...
            if (canAfford)
            {
                MessageQueue.EnqueueDebugging(Info.Name + " 尝试购买 " + Product.Info.FriendlyName + " x " + Quantity + " - 足够的货币.");
            ...
            MessageQueue.EnqueueDebugging(Info.Name + " 尝试购买 " + Product.Info.FriendlyName + " x " + Quantity + " - Purchases Sent!");
            ReceiveChat("您购买的商品已发送到您的邮箱.", ChatType.Hint);
        }
        ...
        public void ItemRentalRequest()
        {
            if (Dead)
            {
                ReceiveChat("死亡时无法租赁物品.", ChatType.System);
                return;
            }

            if (ItemRentalPartner != null)
            {
                ReceiveChat("已经在将物品租赁给另一个玩家.", ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat("面对您想要租赁的玩家.", ChatType.System);
                return;
            }

            if (Info.RentedItems.Count >= 3)
            {
                ReceiveChat("无法一次租赁3个以上的物品.", ChatType.System);
                return;
            }

            if (targetPlayer.Info.HasRentedItem)
            {
                ReceiveChat($"{targetPlayer.Name} 目前无法租赁物品.", ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat("无法租赁自己的物品.", ChatType.System);
                return;
            }

            if (targetPlayer.Dead)
            {
                ReceiveChat($"无法将物品租赁给{targetPlayer.Name}死了.", ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat($"{targetPlayer.Name} 目前很忙，请尽快重试.", ChatType.System);
                return;
            }
        ...
        public void DepositRentalItem(int from, int to)
        {
            ...
            {
                ReceiveChat($"无法租赁{item.FriendlyName} until {item.RentalInformation.ExpiryDate}", ChatType.System);
                Enqueue(packet);
                return;
            }
        ...
        public void RetrieveRentalItem(int from, int to)
        {
            ...
            {
                ReceiveChat("物品太重而无法查找.", ChatType.System);
                Enqueue(packet);
                return;
            }
        ...
        public void ItemRentalLockItem()
        {
            ...
            if (ItemRentalItemLocked && ItemRentalPartner.ItemRentalFeeLocked)
                ...
                ItemRentalPartner.ReceiveChat($"{Name} 已经锁定了租赁物品.", ChatType.System);
        ...
        public void ConfirmItemRental()
        {
            ...
            {
                ReceiveChat($"{ItemRentalPartner.Name}无法收到该物品.", ChatType.System);
            ...
            {
                ReceiveChat("你无法收到金币.", ChatType.System);
                Enqueue(new S.CancelItemRental());

                ItemRentalPartner.ReceiveChat($"{Name}你无法收到金币.", ChatType.System);
                ItemRentalPartner.Enqueue(new S.CancelItemRental());
            ...
            ItemRentalPartner.ReceiveChat($"你租赁了 {item.FriendlyName} 从{Name}到{item.RentalInformation.ExpiryDate}", ChatType.System);

            GainGold(ItemRentalPartner.ItemRentalFeeAmount);
            ReceiveChat($"已收到{ItemRentalPartner.ItemRentalFeeAmount} 商品的租金.", ChatType.System);
        ...
        private bool AddHero(HeroInfo hero)
        {
            ...
            if (heroCount >= Info.MaximumHeroCount)
            {
                ReceiveChat(string.Format("你不能再召唤英雄了."), ChatType.Hint);
                return false;
            }
            ...
            for (int i = 0; i < Info.Heroes.Length; i++)
            {
                ...
                {
                    ReceiveChat(string.Format("英雄已添加到你的英雄存储中."), ChatType.Hint);
