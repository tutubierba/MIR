...
namespace Server.MirObjects
{
    public sealed class GuildObject
    {
        ...
        public bool ChangeRank(PlayerObject self, string memberName, byte rankIndex, string rankName = "Members")
        {
            ...
            {
                self.ReceiveChat(String.Format("行会掌门至少需要等级{0}", Settings.Guild_RequiredLevel), ChatType.System);
                return false;
            }
            ...
            if (MemberRank.Index == 0)
            {
                ...
                {
                    self.ReceiveChat("行会至少需要两个掌门人.", ChatType.System);
                    return false;
                }
                ...
                self.ReceiveChat("至少需要一位掌门在线.", ChatType.System);
                return false;
            }
        ...
        public bool NewRank(PlayerObject Self)
        {
            ...
            {
                Self.ReceiveChat("无法提升更高的职位了.", ChatType.System);
                return false;
            }
            int NewIndex = Ranks.Count > 1? Ranks.Count -1: 1;
            GuildRank NewRank = new GuildRank(){Index = NewIndex, Name = String.Format("职位-{0}", NewIndex), Options = (GuildRankOptions)0};
        ...
        public bool ChangeRankOption(PlayerObject Self, byte RankIndex, int Option, string Enabled)
        {
            ...
            {
                Self.ReceiveChat("职位未找到!", ChatType.System);
                return false;
            }
            if (Self.MyGuildRank.Index >= RankIndex)
            {
                Self.ReceiveChat("不能改变自己的职位!", ChatType.System);
                return false;
            }
            ...
            Ranks[RankIndex].Options = Enabled == "是" ? Ranks[RankIndex].Options |= (GuildRankOptions)(1 << Option) : Ranks[RankIndex].Options ^= (GuildRankOptions)(1 << Option);
        ...
        public bool ChangeRankName(PlayerObject Self, string RankName, byte RankIndex)
        {
            ...
            {
                Self.ReceiveChat("你没有权限.", ChatType.System);
                return false;
            }
        ...
        public bool DeleteMember(PlayerObject Kicker, string membername)
        {
            ...
            {
                Kicker.ReceiveChat("你没有权限.", ChatType.System);
                return false;
            }
            ...
            if (MemberRank.Index == 0)
            {
                if (MemberRank.Members.Count < 2)
                {
                    Kicker.ReceiveChat("掌门无法退出行会.", ChatType.System);
                    return false;
                }
                for (int i = 0; i < MemberRank.Members.Count; i++)
                    if ((MemberRank.Members[i].Online) && (MemberRank.Members[i] != Member))
                        goto AllOk;
                Kicker.ReceiveChat("至少需要一位掌门在线.", ChatType.System);
                return false;
            }
        ...
        public void MemberDeleted(string name, PlayerObject formerMember, bool kickSelf)
        {
            ...
            if (formerMember != null)
            {
                ...
                formerMember.ReceiveChat(kickSelf ? "你离开了行会" : "你被移除出行会.", ChatType.Guild);
    ...
    public class GuildAtWar
    {
        ...
        public void EndWar()
        {
            ...
            GuildA.SendMessage(string.Format("{0}行会，结束了战争.", GuildB.Name), ChatType.Guild);
            GuildB.SendMessage(string.Format("{0}行会，结束了战争.", GuildA.Name), ChatType.Guild);
