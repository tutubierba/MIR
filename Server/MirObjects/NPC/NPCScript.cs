...
namespace Server.MirObjects
{
    public class NPCScript
    {
        ...
        private void ParseGoods(IList<string> lines)
        {
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                while (++i < lines.Count)
                {
                    ...
                    {
                        MessageQueue.Enqueue(string.Format("找不到物品：{0}，文件：{0}", lines[i], FileName));
                        continue;
                    }
        ...
        private void ParseCrafting(IList<string> lines)
        {
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                while (++i < lines.Count)
                {
                    ...
                    if (recipe == null)
                    {
                        MessageQueue.Enqueue(string.Format("找不到配方：{0}，文件：{0}", lines[i], FileName));
                        continue;
                    }

                    if (recipe.Ingredients.Count == 0)
                    {
                        MessageQueue.Enqueue(string.Format("找不到成分：{0}，文件：{0}", lines[i], FileName));
                        continue;
                    }
        ...
        public void Call(PlayerObject player, uint objectID, string key)
        {
            ...
            if (!player.NPCDelayed)
            {
                if (key != MainKey)
                {
                    ...
                    if (!found)
                    {
                        MessageQueue.Enqueue(string.Format("玩家:{0}被阻止访问NPC密钥: '{1}' ", player.Name, key));
                        return;
                    }
                }
            }
        ...
        private void ProcessSpecial(PlayerObject player, NPCPage page)
        {
            ...
            switch (key)
            {
                ...
                case RefineKey:
                    ...
                    {
                        player.ReceiveChat("你已经在精炼物品.", ChatType.System);
                ...
                case GuildCreateKey:
                    ...
                    {
                        player.ReceiveChat(String.Format("你必须至少达到{0}级才能创建行会.", Settings.Guild_RequiredLevel), ChatType.System);
                    }
                ...
                case RequestWarKey:
                    if (player.MyGuild != null)
                    {
                        ...
                        {
                            player.ReceiveChat("你必须是领袖才能发动战争.", ChatType.System);
                            return;
                        }
        ...
        public void Craft(PlayerObject player, ulong index, ushort count, int[] slots)
        {
            ...
            {
                player.ReceiveChat("工艺尝试失败.", ChatType.System);
            }
            else
            {
                //Give Item
