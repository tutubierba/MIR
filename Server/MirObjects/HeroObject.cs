        ...
        public override void UseItem(ulong id)
        {
            ...
            switch (item.Info.Type)
            {
                case ItemType.Potion:
                    switch (item.Info.Shape)
                    {
                        ...
                        case 2: //MysteryWater
                            if (UnlockCurse)
                            {
                                ReceiveChat("你取消了物品的诅咒。", ChatType.Hint);
                                Owner.Enqueue(p);
                                return;
                            }
                            ReceiveChat("你取消了物品的诅咒.", ChatType.Hint);
                ...
                case ItemType.Scroll:
                    ...
                    switch (item.Info.Shape)
                    {
                        ...
                        case 4: //RepairOil
                            ...
                            ReceiveChat("你的武器已经修理好了", ChatType.Hint);
                        ...
                        case 6: //ResurrectionScroll
                            ...
                            {
                                ReceiveChat(string.Format("此地图无法使用"), ChatType.System);
                                Owner.Enqueue(p);
                                return;
                            }
                            if (Dead)
                ...
                case ItemType.Food:
                    ...
                    ReceiveChat("你的坐骑体力得到了恢复.", ChatType.Hint);
                    Owner.Enqueue(new S.ItemRepaired { UniqueID = temp.UniqueID, MaxDura = temp.MaxDura, CurrentDura = temp.CurrentDura });
                ...
                case ItemType.MonsterSpawn:
                    ...
                    if (spawnAsPet)
                    {
                        ...
                        {
                            ReceiveChat("已经达到了最大宠物数量.", ChatType.Hint);
                            Owner.Enqueue(p);
                            return;
                        }
