...
namespace Server.MirObjects
{
    public class Reporting
    {
        ...
        {
            string message = $"移动地图 {oldMap.FileName} => {newMap.FileName}";
        ...
        {
            string message = $"物品拆分 - {item.Info.Name} 从 x{item.Count + newItem.Count}到 x{item.Count} in {grid} - 创建新的物品 ({newItem.UniqueID}) x {newItem.Count}";
        ...
        {
            string message = $"物品合并 - {fromItem.Info.Name} 和 {toItem.Info.Name}到 {slotFrom} ({gridFrom}) to {slotTo} ({gridTo}) ({toItem.UniqueID})";
        ...
        {
            string message = $"物品组合 - {fromItem.Info.Name} with {toItem.Info.Name} from {slotFrom} to {slotTo} in {grid} ({toItem.UniqueID})";
        ...
        {
            string message = $"物品移动 - {(item != null ? item.Info.Name : "Empty")} from {from}:{slotFrom} to {to}:{slotTo} ({item?.UniqueID}) {info}";
