                        ...
                        foreach (KeyValuePair<ConquestGuildFlagInfo, Dictionary<GuildObject, int>> item in ControlPoints)
                        {
                            ...
                            for (int i = 0; i < ConquestMap.Players.Count; i++)
                            {
                                ...
                                if (points == 0)
                                {
                                    ...
                                    ConquestMap.Players[i].MyGuild.SendOutputMessage(string.Format("{1}获得{0:P0}的控制权", ((double)controlFlagPoints[ConquestMap.Players[i].MyGuild] / MAX_CONTROL_POINTS), controlFlag.Info.Name));
                                }
                            ...
                            if (pointsChanged)
                            {
                                ...
                                if (tempWinning != controlFlag.Guild)
                                {
                                    ...
                                    for (int j = 0; j < ConquestMap.Players.Count; j++)
                                    {
                                        ConquestMap.Players[j].ReceiveChat(string.Format("{0}在{2}上捕获了{1}", tempWinning.Name, controlFlag.Info.Name, Info.Name), ChatType.System);
                                    }
                                }
                            }
                        }
                ...
                case ConquestGame.KingOfHill:
                    {
                        ...
                        for (int i = 0; i < ConquestMap.Players.Count; i++)
                        {
                            if (ConquestMap.Players[i].WarZone && ConquestMap.Players[i].MyGuild != null && !ConquestMap.Players[i].Dead && Functions.InRange(Info.KingLocation, ConquestMap.Players[i].CurrentLocation, Info.KingSize))
                            {
                                ...
                                if (points == 0)
                                {
                                    ...
                                    ConquestMap.Players[i].MyGuild.SendOutputMessage(string.Format("{1}获得{0:P0}的控制权", ((double)KingPoints[ConquestMap.Players[i].MyGuild] / MAX_KING_POINTS), Info.Name));
                                }
                                ...
                                foreach (var guild in guilds)
                                {
                                    ...
                                    {
                                        KingPoints[guild] -= 1;
                                        guild.SendOutputMessage(string.Format(" {1}失去对{0:P0}的控制权", ((double)KingPoints[guild] / MAX_KING_POINTS), Info.Name));
                                    }
                                }
                        ...
                        if (pointsChanged)
                        {
                            ...
                            if (tempWinning != Guild)
                            {
                                ...
                                for (int j = 0; j < ConquestMap.Players.Count; j++)
                                {
                                    ConquestMap.Players[j].ReceiveChat(string.Format("{0}捕获了山丘", tempWinning.Name), ChatType.System);
                                }
                            }
                        }
                    }
