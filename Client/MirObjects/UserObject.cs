    ...
    public class UserObject : PlayerObject
    {
        ...
        public void GetMaxGain(UserItem item)
        {
            ...
            if (item.Info.Type == ItemType.Amulet)
            {
                ...
                if (min == 0)
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(FreeSpace(Inventory) == 0 ?  "背包已经满了" : "太重了，你已经背不动了", ChatType.System);
            ...
            if (item.Info.StackSize > 1)
            {
                ...
                if (min == 0)
                {
                    GameScene.Scene.ChatDialog.ReceiveChat("背包已经满了", ChatType.System);
                    item.Count = 0;
                }
            }
            else
