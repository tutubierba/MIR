    ...
    public sealed class GameShopCell : MirImageControl
    {
        ...
        public void BuyProduct()
        {
            ...
            if (Item.CreditPrice * Quantity <= GameScene.Credit)
            {
                ...
                messageBox = new MirMessageBox(string.Format("你确定要花费{2}积分购买{0}X{1}吗？", Item.Info.FriendlyName, Quantity, CreditCost, Item.Count), MirMessageBoxButtons.YesNo);
            }
            else
            { //Needs to attempt to pay with gold and credits
                if (GameScene.Gold >= (((Item.GoldPrice * Quantity) / (Item.CreditPrice * Quantity)) * ((Item.CreditPrice * Quantity) - GameScene.Credit)))
                {
                    ...
                    if (CreditCost == 0)
                    {
                        messageBox = new MirMessageBox(string.Format("你确定要花费{2}金币购买{0}X{1}吗？", Item.Info.FriendlyName, Quantity, GoldCost, Item.Count), MirMessageBoxButtons.YesNo);
                    }
                    else
                    {
                        messageBox = new MirMessageBox(string.Format("你确定要花费{2}积分和{3}金币购买{0}X{1}吗？", Item.Info.FriendlyName, Quantity, CreditCost, GoldCost, Item.Count), MirMessageBoxButtons.YesNo);
                    }
                }
                else
                {
                    GameScene.Scene.ChatDialog.ReceiveChat("没有足够的金币", ChatType.System);
                    return;
                }
