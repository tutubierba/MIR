    ...
    public sealed class MLibrary
    {
        ...
        public void Initialize()
        {
            ...
            try
            {
                ...
                if (currentVersion < 2)
                {
                    System.Windows.Forms.MessageBox.Show("客户端格式版本错误，需要版本: " + LibVersion.ToString() + " 当前版本: " + currentVersion.ToString() + ".", _fileName, System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error, System.Windows.Forms.MessageBoxDefaultButton.Button1);
        ...
        {
            //图片实际长度就384张  死亡的监测动画是704张 所以返回了 怪物也消失了 没有图片能放了
            if (!CheckImage(index))
                return;
