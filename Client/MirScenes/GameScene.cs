...
namespace Client.MirScenes
{
    public sealed class GameScene : MirScene
    {
        ...
        public GameScene()
        {
            ...
            for (int i = 0; i < OutputLines.Length; i++)
                OutputLines[i] = new MirLabel
                {
                    ...
                    Location = new Point(20, 25 + i * 13), //获取经验值物品提示位置
                    OutLine = true,
                };
        ...
        public void OutputMessage(string message, OutputMessageType type = OutputMessageType.Normal)
        {
            ...
            if (OutputMessages.Count > 10)  //获取经验值物品提示行数
        ...
        public void UseSpell(int key)
        {
            ...
            {
                ChatDialog.ReceiveChat("你必须佩戴一把合适的武器才能释放这个技能", ChatType.System);
                return;
            }
            ...
            switch (magic.Spell)
            {
                ...
                case Spell.Thrusting:
                    ...
                    ChatDialog.ReceiveChat(prefix + (actor.Thrusting ? "开启刺杀剑术" : "关闭刺杀剑术."), ChatType.Hint);
                ...
                case Spell.HalfMoon:
                    ...
                    ChatDialog.ReceiveChat(prefix + (actor.HalfMoon ? "开启半月弯刀" : "关闭半月弯刀."), ChatType.Hint);
                    ToggleTime = CMain.Time + 1000;
                ...
                case Spell.CrossHalfMoon:
                    ...
                    ChatDialog.ReceiveChat(prefix + (actor.CrossHalfMoon ? "开启十方斩" : "关闭十方斩."), ChatType.Hint);
                    ToggleTime = CMain.Time + 1000;
                ...
                case Spell.DoubleSlash:
                    ...
                    ChatDialog.ReceiveChat(prefix + (actor.DoubleSlash ? "开启顺劈斩" : "关闭顺劈斩"), ChatType.Hint);
                    ToggleTime = CMain.Time + 1000;
        ...
        public void QuitGame()
        {
            if (CMain.Time >= LogTime)
            {
                ...
                MirMessageBox messageBox = new MirMessageBox("退出游戏？", MirMessageBoxButtons.YesNo);
                messageBox.YesButton.Click += (o, e) => Program.Form.Close();
                messageBox.Show();
            }
            else
            {
                ChatDialog.ReceiveChat(string.Format("战斗中无法退出游戏，剩余：{0} 秒", (LogTime - CMain.Time) / 1000), ChatType.System);
            }
        }
        public void LogOut()
        {
            ...
            {
                //If Last Combat < 10 CANCEL
                MirMessageBox messageBox = new MirMessageBox("返回角色选择？", MirMessageBoxButtons.YesNo);
        ...
        public override void Process()
        {
            ...
            if (ShowReviveMessage && CMain.Time > User.DeadTime && User.CurrentAction == MirAction.Dead)
            {
                ...
                MirMessageBox messageBox = new MirMessageBox("你已经死亡，在安全区复活？", MirMessageBoxButtons.YesNo, false);
        ...
        private void SearchMapResult(S.SearchMapResult info)
        {
            ...
            {
                MirMessageBox messageBox = new MirMessageBox("未找到.", MirMessageBoxButtons.OK);
        ...
        private void UserLocation(S.UserLocation p)
        {
            ...
            {
                ReceiveChat(new S.Chat { Message = "移动", Type = ChatType.System });
            }
        ...
        private void ShareQuest(S.ShareQuest p)
        {
            ...
            MirMessageBox messageBox = new MirMessageBox(string.Format("{0}共享了一个任务给你，接受吗？", p.SharerName), MirMessageBoxButtons.YesNo);
        ...
        private void ReturnToLogin(S.ReturnToLogin p)
        {
            ...
            Dispose();
            MirMessageBox.Show("你正在观看的玩家已退出游戏");
        }
        ...
        private void ChangeAMode(S.ChangeAMode p)
        {
            ...
            {
                case AttackMode.Peace:
                    ChatDialog.ReceiveChat("[和平攻击模式]", ChatType.Hint);
                    break;
                case AttackMode.Group:
                    ChatDialog.ReceiveChat("[编组攻击模式]", ChatType.Hint);
                    break;
                case AttackMode.Guild:
                    ChatDialog.ReceiveChat("[行会攻击模式]", ChatType.Hint);
                    break;
                case AttackMode.EnemyGuild:
                    ChatDialog.ReceiveChat("[敌对攻击模式]", ChatType.Hint);
                    break;
                case AttackMode.RedBrown:
                    ChatDialog.ReceiveChat("[善恶攻击模式]", ChatType.Hint);
                    break;
                case AttackMode.All:
                    ChatDialog.ReceiveChat("[全体攻击模式]", ChatType.Hint);
                    break;
            }
        }
        private void ChangePMode(S.ChangePMode p)
        {
            ...
            {
                case PetMode.Both:
                    ChatDialog.ReceiveChat("[宠物：攻击]", ChatType.Hint);
                    break;
                case PetMode.MoveOnly:
                    ChatDialog.ReceiveChat("[宠物：跟随]", ChatType.Hint);
                    break;
                case PetMode.AttackOnly:
                    ChatDialog.ReceiveChat("[宠物：停止]", ChatType.Hint);
                    break;
                case PetMode.None:
                    ChatDialog.ReceiveChat("[宠物：休息]", ChatType.Hint);
                    break;
            }
        }
        ...
        private void GainedItem(S.GainedItem p)
        {
            ...
            OutputMessage(string.Format("你获得了{0}", p.Item.FriendlyName));
        }
        ...
        private void GainedGold(S.GainedGold p)
        {
            ...
            OutputMessage(string.Format("你获得了{0:###,###,###}金币", p.Gold));
        }
        ...
        private void GainedCredit(S.GainedCredit p)
        {
            ...
            Credit += p.Credit;
            SoundManager.PlaySound(SoundList.Gold);
            OutputMessage(string.Format("你获得了{0:###,###,###}积分", p.Credit));
        }
        ...
        private void Struck(S.Struck p)
        {
            ...
            NextRunTime = CMain.Time + 2500; //受攻击后起跑时间，单位：毫秒
                ...
                for (int i = MapControl.Objects.Count - 1; i >= 0; i--)
                {
                    ...
                    switch (p.Type)
                    {
                        ...
                        case DamageType.Miss:
                            obj.Damages.Add(new Damage("防御", 1200, obj.Race == ObjectType.Player ? Color.LightCoral : Color.LightGray, 50));
                            break;
        ...
        private void DuraChanged(S.DuraChanged p)
        {
            ...
            if (item.CurrentDura == 0)
            {
                ...
                {
                    case ItemType.Mount:
                        ChatDialog.ReceiveChat(string.Format("{0}现在非常饥饿.", item.Info.FriendlyName), ChatType.System);
                        break;
                    default:
                        ChatDialog.ReceiveChat(string.Format("{0}的持久降到0了.", item.Info.FriendlyName), ChatType.System);
                        break;
                }
        ...
        {
            OutputMessage(string.Format("英雄获得经验值{0}", p.Amount));
        ...
        private void LevelChanged(S.LevelChanged p)
        {
            ...
            ChatDialog.ReceiveChat("愿上帝护佑着你！恭喜你升级啦！", ChatType.LevelUp); 
        }
        ...
        {
            MirInputBox inputBox = new MirInputBox("请输入.");
        ...
        private void DeleteGroup()
        {
            ...
            ChatDialog.ReceiveChat("退出队伍.", ChatType.Group);
        }
        ...
        private void DeleteMember(S.DeleteMember p)
        {
            ...
            ChatDialog.ReceiveChat(string.Format("-{0}退出队伍.", p.Name), ChatType.Group);
        }

        private void GroupInvite(S.GroupInvite p)
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("你要和{0}组队吗？", p.Name), MirMessageBoxButtons.YesNo);
        ...
        private void AddMember(S.AddMember p)
        {
            ...
            ChatDialog.ReceiveChat(string.Format("-{0}加入队伍.", p.Name), ChatType.Group);
        }
            ...
            switch (p.Spell)
            {
                ...
                case Spell.FlamingSword:
                    ...
                    if (actor.FlamingSword)
                        ChatDialog.ReceiveChat(prefix + "你的武器因精神火球而灼热", ChatType.Hint);
                    else
                        ChatDialog.ReceiveChat(prefix + "精神火球消失", ChatType.System);
                    break;
            }
        }
        ...
        private void UserAttackMove(S.UserAttackMove p)//战士 Skill - SlashingBurst
        {
            MapControl.NextAction = 0;
            if (User.CurrentLocation == p.Location && User.Direction == p.Direction) return;
        ...
        private void MarketFail(S.MarketFail p)
        {
            ...
            {
                case 0:
                    MirMessageBox.Show("无法在死亡状态下使用拍卖行.");
                    break;
                case 1:
                    MirMessageBox.Show("无法在使用中购买.");
                    break;
                case 2:
                    MirMessageBox.Show("物品已售出");
                    break;
                case 3:
                    MirMessageBox.Show("无法购买过期物品.");
                    break;
                case 4:
                    MirMessageBox.Show("没有足够的金币");
                    break;
                case 5:
                    MirMessageBox.Show("没有足够的负重或背包空间.");
                    break;
                case 6:
                    MirMessageBox.Show("无法购买自己出售的物品.");
                    break;
                case 7:
                    MirMessageBox.Show("离拍卖行商人距离过远.");
                    break;
                case 8:
                    MirMessageBox.Show("手续费不足，无法上架拍卖品.");
                    break;
                case 9:
                    MirMessageBox.Show("底价过低无法出售.");
                    break;
                case 10:
                    MirMessageBox.Show("拍卖已经结束了.");
                    break;
            }
        ...
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("是否要加入{0}行会?", p.Name), MirMessageBoxButtons.YesNo);
        ...
        private void GuildNameRequest(S.GuildNameRequest p)
        {
            ...
            inputBox.OKButton.Click += (o, e) =>
            {
                ...
                {
                    ChatDialog.ReceiveChat("行会名称中不能包涵\\符号!", ChatType.System);
        ...
        {
            MirInputBox inputBox = new MirInputBox("输入你要发动行会战争的行会名称.");
        ...
        private void GuildMemberChange(S.GuildMemberChange p)
        {
            switch (p.Status)
            {
                case 0: // logged of
                    ...
                    break;
                case 1: // logged on
                    ChatDialog.ReceiveChat(String.Format("{0}上线了.", p.Name), ChatType.Guild);
                ...
                case 3://kicked member
                    ChatDialog.ReceiveChat(String.Format("{0}被踢出出行会.", p.Name), ChatType.Guild);
                    GuildDialog.MembersChanged = true;
                    break;
                case 4://member left
                    ChatDialog.ReceiveChat(String.Format("{0}离开行会.", p.Name), ChatType.Guild);
                    GuildDialog.MembersChanged = true;
                    break;
        ...
        private void GuildStorageGoldChange(S.GuildStorageGoldChange p)
        {
            ...
            {
                case 0:
                    ChatDialog.ReceiveChat(String.Format("向行会捐赠了{1}金币.", p.Name, p.Amount), ChatType.Guild);
                    GuildDialog.Gold += p.Amount;
                    break;
                case 1:
                    ChatDialog.ReceiveChat(String.Format("{0}从行会资金中取出{1}金币.", p.Name, p.Amount), ChatType.Guild);
        ...
        {            
            ...
            NewHeroDialog.ArcherButton.Visible = p.CanCreateClass[(int)MirClass.弓箭手];
        ...
        private void NewHero(S.NewHero p)
        {
            ...
            {
                case 0:
                    MirMessageBox.Show("禁止创建英雄.");
                    NewHeroDialog.Hide();
                    break;
                case 1:
                    MirMessageBox.Show("请重新输入一个英雄名字.");
                ...
                case 3:
                    ...
                    break;
                case 4:
                    MirMessageBox.Show("你无法创建更多的英雄了.");
                    NewHeroDialog.Hide();
                    break;
                case 5:
                    MirMessageBox.Show("名字已存在.");
                    NewHeroDialog.NameTextBox.SetFocus();
                    break;
                case 6:
        ...
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("你接受{0}的求婚吗？", p.Name), MirMessageBoxButtons.YesNo);
        ...
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("你接受{0}的离婚请求吗？", p.Name), MirMessageBoxButtons.YesNo);
        ...
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("{0} (等级{1})职业：{2}，向你拜师.", p.Name, p.Level, GameScene.User.Class.ToString()), MirMessageBoxButtons.YesNo);
        ...
        private void GuildBuffList(S.GuildBuffList p)
        {
            ...
            if (GuildDialog.EnabledBuffs.Any(e => e.Active))
            {
                if (buff == null)
                {
                    buff = new ClientBuff { Type = BuffType.Guild, ObjectID = User.ObjectID, Caster = "行会", Infinite = true, Values = new int[0] };
        ...
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("玩家{0}向你发起交易.", p.Name), MirMessageBoxButtons.YesNo);
        ...
        private void Awakening(S.Awakening p)
        {
            ...
            switch (p.result)
            {
                case -4:
                    ...
                    break;
                case -3:
                    messageBox = new MirMessageBox("没有足够的金币", MirMessageBoxButtons.OK);
                    MapControl.AwakeningAction = false;
                    break;
                case -2:
                    messageBox = new MirMessageBox("这个物品无法觉醒.", MirMessageBoxButtons.OK);
                    MapControl.AwakeningAction = false;
                    break;
                case -1:
                    messageBox = new MirMessageBox("已达到最高觉醒等级.", MirMessageBoxButtons.OK);
                    MapControl.AwakeningAction = false;
                    break;
                case 0:
        ...
        {
            MirInputBox inputBox = new MirInputBox("输入收件人姓名.");
        ...
        private void ParcelCollected(S.ParcelCollected p)
        {
            ...
            {
                case -1:
                    MirMessageBox messageBox = new MirMessageBox(string.Format("没有可收取的邮件."), MirMessageBoxButtons.OK);
                    messageBox.Show();
                    break;
                case 0:
                    messageBox = new MirMessageBox(string.Format("所有邮件都已收取."), MirMessageBoxButtons.OK);
                    messageBox.Show();
                    break;
                case 1:
        ...
        private void RequestReincarnation()
        {
            ...
            {
                MirMessageBox messageBox = new MirMessageBox("你想要被复活吗？", MirMessageBoxButtons.YesNo);
        ...
        private void NewIntelligentCreature(S.NewIntelligentCreature p)
        {
            ...
            MirInputBox inputBox = new MirInputBox("请给你的宠物取一个名字.");
            ...
            if (HoverItem.Info.Durability > 0 && !hideDura)
            {
                switch (HoverItem.Info.Type)
                {
                    case ItemType.Amulet:
                        text += string.Format("\n耐久:{0}/{1}", HoverItem.CurrentDura, HoverItem.MaxDura);
                        break;
                    case ItemType.Ore:
                        text += string.Format("\n纯度:{0}", Math.Floor(HoverItem.CurrentDura / 1000M));
                        break;
                    case ItemType.Meat:
                        text += string.Format("\n品质:{0}", Math.Floor(HoverItem.CurrentDura / 1000M));
                        break;
                    case ItemType.Mount:
                        text += string.Format("\n饥饿:{0}/{1}", HoverItem.CurrentDura, HoverItem.MaxDura);
                        break;
                    case ItemType.Food:
                        text += string.Format("\n活力:{0}", HoverItem.CurrentDura);
                        break;
                    case ItemType.Gem:
        ...
        public MirControl AttackInfoLabel(UserItem item, bool Inspect = false, bool hideAdded = false)
        {
            ...
            if (minValue > 0 && realItem.Type == ItemType.Gem)
            {
                ...
                {
                    default:
                        text = string.Format("增加{0}持久", minValue / 1000);
                        break;
                    case 8:
                        text = string.Format("封印中{0}", Functions.PrintTimeSpanFromSeconds(minValue * 60));
                        break;
                }
            ...
            if (minValue != 0 || addValue != 0)
            {
                ...
                {
                    text = string.Format("物品爆率+{0}%", minValue + addValue);
                }
                else
                {
            ...
            if (minValue != 0 || maxValue != 0 || addValue != 0)
            {
                ...
                if (HoverItem.Info.Type != ItemType.Gem)
                {
                    ...
                    text = string.Format("增加{0}攻速", minValue + maxValue + addValue);
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                count++;
                if (HoverItem.Info.Type != ItemType.Gem)
                    text = string.Format(addValue > 0 ? "毒药效果：+{0} (+{1})" : "毒药效果：+{0}", minValue + addValue, addValue);
                else
                    text = string.Format("增加{0}毒药效果", minValue + maxValue + addValue);
            ...
            if ((minValue > 0 || maxValue > 0 || addValue > 0) && (realItem.Type != ItemType.Gem))
            {
                ...
                MirLabel CRITICALRATELabel = new MirLabel
                {
                    ...
                    Text = string.Format(addValue > 0 ? "暴击：+{0} (+{1})" : "暴击：+{0}", minValue + addValue, addValue)
                };
            ...
            if ((minValue > 0 || maxValue > 0 || addValue > 0) && (realItem.Type != ItemType.Gem))
            {
                ...
                MirLabel ReflectLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("反伤几率：{0}", minValue)
                };
            ...
            if ((minValue > 0 || maxValue > 0 || addValue > 0) && (realItem.Type != ItemType.Gem))
            {
                ...
                MirLabel HPdrainLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("HP吸取：{0}%", minValue)
                };
            ...
            if (minValue != 0 || maxValue != 0 || addValue != 0)
            {
                ...
                string negative = "+";
                if (addValue < 0) negative = "";
                text = string.Format(addValue != 0 ? "经验值：" + plus + "{0}% ({2}{1}%)" : "经验值：" + plus + "{0}%", minValue + addValue, addValue, negative);
            ...
            if (minValue != 0 || maxValue != 0 || addValue != 0)
            {
                ...
                string negative = "+";
                if (addValue < 0) negative = "";
                text = string.Format(addValue != 0 ? "物品爆率：" + plus + "{0}% ({2}{1}%)" : "物品爆率：" + plus + "{0}%", minValue + addValue, addValue, negative);
        ...
        public MirControl DefenceInfoLabel(UserItem item, bool Inspect = false, bool hideAdded = false)
        {
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                if (fishingItem)
                {
                    ...
                    {
                        ACLabel.Text = string.Format("蚕食率 + " + (addValue > 0 ? "{0}~{1}% (+{2})" : "{0}~{1}%"), minValue, maxValue + addValue);
                    }
                    else if (HoverItem.Info.Type == ItemType.Finder)
                    {
                        ACLabel.Text = string.Format("自动钓鱼几率 + " + (addValue > 0 ? "{0}~{1}% (+{2})" : "{0}~{1}%"), minValue, maxValue + addValue);
                    }
                    else
                    {
                        ACLabel.Text = string.Format("探测率 + " + (addValue > 0 ? "{0}% (+{1})" : "{0}%"), maxValue, maxValue + addValue);
                    }
                }
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel MAXHPRATELabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("生命恢复：+{0}", minValue + addValue)
                };
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel MAXACRATE = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("防御上限+{0}%", minValue + addValue)
                };
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel MAXMACRATELabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("魔法防御上限+{0}%", minValue + addValue)
                };
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel MANA_RECOVERYLabel = new MirLabel
                {
                    ...
                    Text = string.Format(addValue > 0 ? "魔法恢复：+{0} (+{1})" : "魔法恢复：+{0}", minValue + addValue, addValue)
                };
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                count++;
                if (HoverItem.Info.Type != ItemType.Gem)
                    text = string.Format(addValue > 0 ? "毒素抗性：+{0} (+{1})" : "毒素抗性：+{0}", minValue + addValue, addValue);
                else
                    text = string.Format("增加{0}毒素抗性", minValue + maxValue + addValue);
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                count++;
                if (HoverItem.Info.Type != ItemType.Gem)
                    text = string.Format(addValue > 0 ? "魔法抗性：+{0} (+{1})" : "魔法抗性：+{0}", minValue + addValue, addValue);
                else
                    text = string.Format("增加{0}魔法抗性", minValue + maxValue + addValue);
        ...
        public MirControl AwakeInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (HoverItem.Awake.GetAwakeLevel() > 0)
            {
                ...
                MirLabel AWAKENAMELabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("{0}觉醒等级：({1})", HoverItem.Awake.Type.ToString(), HoverItem.Awake.GetAwakeLevel())
                };
            ...
            if (HoverItem.Awake.GetAwakeValue() > 0)
            {
                ...
                MirLabel AWAKE_TOTAL_VALUELabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format(realItem.Type != ItemType.Armour ? "觉  醒：{0} +{1}~{2}" : "觉  醒：{0}上限 +{1}", HoverItem.Awake.Type.ToString(), HoverItem.Awake.GetAwakeValue(), HoverItem.Awake.GetAwakeValue())
                };
            ...
            if (HoverItem.Awake.GetAwakeLevel() > 0)
            {
                ...
                for (int i = 0; i < HoverItem.Awake.GetAwakeLevel(); i++)
                {
                    MirLabel AWAKE_LEVEL_VALUELabel = new MirLabel
                    {
                        ...
                        OutLine = true,
                        Parent = ItemLabel,
                        Text = string.Format(realItem.Type != ItemType.Armour ? "等级{0}：{1} +{2}~{3}" : "等级{0}：{1}上限 +{2}~{3}", i + 1, HoverItem.Awake.Type.ToString(), HoverItem.Awake.GetAwakeLevelValue(i), HoverItem.Awake.GetAwakeLevelValue(i))
                    };
        ...
        public MirControl SocketInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            for (int i = 0; i < item.Slots.Length; i++)
            {
                ...
                MirLabel SOCKETLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("兽栏：{0}", item.Slots[i] == null ? "空" : item.Slots[i].FriendlyName)
                };
        ...
        public MirControl NeedInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (realItem.RequiredAmount > 0)
            {
                ...
                switch (realItem.RequiredType)
                {
                    ...
                    case RequiredType.MinMAC:
                        text = string.Format("需要基础魔法防御：{0}", realItem.RequiredAmount);
            ...
            if (realItem.RequiredClass != RequiredClass.全职业)
            {
                ...
                MirLabel CLASSLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("允许佩戴职业：{0}", realItem.RequiredClass)
                };
        ...
        public MirControl BindInfoLabel(UserItem item, bool Inspect = false, bool hideAdded = false)
        {
            ...
            else if (HoverItem.SoulBoundId != -1)
            {
                ...
                MirLabel BOELabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = "绑定至： " + GetUserName((uint)HoverItem.SoulBoundId)
                };
            ...
            if (HoverItem.SealedInfo != null)
            {
                ...
                if (remainingSeconds > 0)
                {
                    ...
                    MirLabel SEALEDLabel = new MirLabel
                    {
                        ...
                        OutLine = true,
                        Parent = ItemLabel,
                        Text = remainingSeconds > 0 ? string.Format("封印中：{0}", Functions.PrintTimeSpanFromSeconds(remainingSeconds)) : ""
                    };
            ...
            if (HoverItem.RentalInformation?.RentalLocked == false)
            {
                ...
                MirLabel OWNERLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = "物品出租人：" + HoverItem.RentalInformation.OwnerName
                };
                ...
                MirLabel RENTALLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = remainingTime > 0 ? string.Format("租期剩余：{0}", Functions.PrintTimeSpanFromSeconds(remainingTime)) : "租期已到"
                };
            ...
            else if (HoverItem.RentalInformation?.RentalLocked == true && HoverItem.RentalInformation.ExpiryDate > CMain.Now)
            {
                ...
                var RentalLockLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = remainingTime > 0 ? string.Format("锁定时间：{0}", Functions.PrintTimeSpanFromSeconds(remainingTime)) : "到期锁定"
                };
        ...
        public MirControl OverlapInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (realItem.Type == ItemType.Gem)
            {
                ...
                {
                    case 1:
                        text = "按住CTRL+鼠标左键使用来维修武器.";
                        break;
                    case 2:
                        text = "按住CTRL+鼠标左键使用来维修盔甲\n和其他可修理物品.";
                        break;
                    case 3:
                    case 4:
                        text = "按住CTRL+鼠标左键放在物品上使用.";
                        break;
                    case 8:
                        text = "按住CTRL+鼠标左键封印物品.";
                        break;
                }
                count++;
            ...
            if (realItem.StackSize > 1 && realItem.Type != ItemType.Gem)
            {
                ...
                MirLabel SPLITUPLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("最大堆叠数量：{0}\n按住Shift并单击鼠标左键可拆分", realItem.StackSize)
                };
        ...
        public MirControl StoryInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            {
                HoverItem.Info.ToolTip = string.Format("你的账号增加了{0}积分.", HoverItem.Info.Price);
            }
        ...
        public void CreateMailLabel(ClientMail mail)
        {
            ...
            MirLabel dateLabel = new MirLabel
            {
                ...
                OutLine = true,
                Parent = MailLabel,
                Text = string.Format("发送日期:{0}", mail.DateSent.ToString("yyyy年MM月dd HH时mm分ss秒"))
            };
        ...
        private void CancelItemRental(S.CancelItemRental p)
        {
            ...
            var messageBox = new MirMessageBox("物品租赁已取消.\r\n" +
                                               "要完成物品租赁，请在整个交易过程中面对另一方.");
            messageBox.Show();
        }
    ...
    public sealed class MapControl : MirControl
    {
        ...
        public static bool AutoRun
        {
            ...
            set
            {
                ...
                if (GameScene.Scene != null)
                    GameScene.Scene.ChatDialog.ReceiveChat(value ? "[自动奔跑:开]" : "[自动奔跑:关]", ChatType.Hint);
            }
        ...
        private static void OnMouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (GameScene.SelectedCell != null)
            {
                ...
                {
                    MirMessageBox messageBox = new MirMessageBox(string.Format("你不能扔掉{0}", cell.Item.FriendlyName), MirMessageBoxButtons.OK);
                    messageBox.Show();
                    GameScene.SelectedCell = null;
                    return;
                }
                ...
                {
                    MirMessageBox messageBox = new MirMessageBox(string.Format("确定要扔掉{0}吗？", cell.Item.FriendlyName), MirMessageBoxButtons.YesNo);
        ...
        private void CheckInput()
        {
            ...
            if (MapObject.TargetObject != null && !MapObject.TargetObject.Dead)
            {
                    ...
                    (!MapObject.TargetObject.Name.EndsWith(")") && MapObject.TargetObject is MonsterObject))
                {
                    ...
                    if (User.Class == MirClass.弓箭手 && User.HasClassWeapon && !User.RidingMount && !User.Fishing)//ArcherTest - non aggressive targets (player / pets)
                    {
                    ...
                    case MouseButtons.Left:
                        ...
                        if (CMain.Shift)
                        {
                            if (CMain.Time > GameScene.AttackTime && CanRideAttack()) //ArcherTest - shift click
                            {
                                ...
                                if (User.Class == MirClass.弓箭手 && User.HasClassWeapon && !User.RidingMount && !User.Poison.HasFlag(PoisonType.Dazed))
                                {
                                    if (target != null)
                                    {
                        ...
                        if (MapObject.MouseObject is MonsterObject && User.Class == MirClass.弓箭手 && MapObject.TargetObject != null && !MapObject.TargetObject.Dead && User.HasClassWeapon && !User.RidingMount) //ArcherTest - range attack
                        {
                            if (Functions.InRange(MapObject.MouseObject.CurrentLocation, User.CurrentLocation, Globals.MaxAttackRange))
                            {
                                if (CMain.Time > GameScene.AttackTime)
            ...
            if (User.Class == MirClass.弓箭手 && User.HasClassWeapon && (MapObject.TargetObject is MonsterObject || MapObject.TargetObject is PlayerObject)) return; //ArcherTest - stop walking
        ...
        public void UseMagic(ClientMagic magic, UserObject actor)
        {
            ...
            switch (magic.Spell)
            {
                ...
                case Spell.SummonSnakes:
                    ...
                    {
                        GameScene.Scene.OutputMessage("释放这个技能需要装备一把弓箭.");
                        actor.ClearMagic();
                        return;
                    }
                    if (actor.NextMagicObject != null)
