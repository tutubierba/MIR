...
namespace Client.MirScenes.Dialogs
{
    ...
    public sealed class GuildDialog : MirImageControl
    {
        ...
        public GuildDialog()
        {
            ...
            String[] Options = { "编辑封号", "招募成员", "开除成员", "存储物品", "取出物品", "行会联盟", "编辑公告", "激活Buff" };
            ...
            else
            {
                ...
                for (int i = 0; i < Buffs.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (Buff.Active)
                                Buffs[i].Info.Text = "冷却.";
                            else
                                Buffs[i].Info.Text = "已到期.";
                        }
                        else
        ...
        public void UpdateActiveStats()
        {
            ...
            foreach (var val in stats.Values)
            {
                ...
                var txt = $"{c} {val.Key}来自于: {val.Value}{(val.Key.ToString().Contains("百分之") ? "%" : "")}.\n";
        ...
        public void CreateHintLabel(byte Id)
        {
            ...
            {
                if (ReqText != "") ReqText += "\n";
                ReqText += "需要点数: " + Buff.PointsRequirement.ToString();
            }
            if (Buff.ActivationCost > 0)
            {
                if (ReqText != "") ReqText += "\n";
                ReqText += "激活需要消耗: " + Buff.ActivationCost.ToString() + "金币.";
                //if (ReqText != "") ReqText += "\n";
            }
            ...
            if (Buff.TimeLimit > 0)
            {
                ...
                {
                    text = string.Format("Buff持续：{0}分钟.", Buff.TimeLimit.ToString());
                }
        ...
        public void OnNewRank(int Index, int SelectedIndex)
        {
            ...
            MirMessageBox messageBox = new MirMessageBox(string.Format("你确定要把{0}的称号更换为{1}吗？", MembersName[Index].Text, Ranks[SelectedIndex].Name), MirMessageBoxButtons.YesNo);
        ...
        public void DeleteMember(int Index)
        {
            ...
            if (LastGuildMsg > CMain.Time) return;
            MirMessageBox messageBox = new MirMessageBox(string.Format("你确定要把{0}开除出行会吗？", MembersName[Index].Text), MirMessageBoxButtons.YesNo);
