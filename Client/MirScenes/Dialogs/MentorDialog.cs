    ...
    public sealed class MentorDialog : MirImageControl
    {
        ...
        public MentorDialog()
        {
            ...
            RemoveButton.Click += (o, e) =>
            {
                if (MentorName == "")
                {
                    GameScene.Scene.ChatDialog.ReceiveChat("你没有师傅", ChatType.System);
                    return;
                }

                MirMessageBox messageBox = new MirMessageBox(string.Format("恩断义绝后一段时间内无法再次使用师徒功能，是否确定？"), MirMessageBoxButtons.YesNo);
