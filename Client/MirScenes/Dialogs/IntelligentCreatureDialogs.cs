...
namespace Client.MirScenes.Dialogs
{
    public sealed class IntelligentCreatureDialog : MirImageControl
    {
        ...
        private void ButtonClick(object sender, EventArgs e)
        {
            ...
            {
                MirInputBox inputBox = new MirInputBox("请输入一个新名字.");
                ...
                inputBox.OKButton.Click += (o1, e1) =>
                {
                    ...
                    {
                        MirMessageBox failedMessage = new MirMessageBox(string.Format("名字长度必须在{0}到{1}个字之间.", Globals.MinCharacterNameLength, Globals.MaxCharacterNameLength), MirMessageBoxButtons.OK);
                        failedMessage.Show();
                    }
                    else
                    {
            ...
            {
                MirInputBox verificationBox = new MirInputBox("输入宠物名字进行验证.");
        ...
        private void DrawCreatureAnimation()
        {
            ...
            var semi = rules.SemiAutoPickupEnabled ? string.Format("{0}x{0} {1}{2}{3}", rules.AutoPickupRange, rules.AutoPickupEnabled ? "自动/" : "", rules.SemiAutoPickupEnabled ? "半自动拾取" : "", rules.MousePickupEnabled ? ", " : "") : "";
            var mouse = rules.SemiAutoPickupEnabled ? string.Format("{0}x{0} 鼠标指向拾取", rules.MousePickupRange) : "";
            ...
            CreatureInfo1.Text = rules.CanProduceBlackStone ? "可生产黑珍珠." : "";
            CreatureInfo2.Text = rules.CanProduceBlackStone ? "可以生产珍珠，购买宠物用品." : "";

            //Expire
            if (GameScene.User.IntelligentCreatures[selectedCreature].Expire == DateTime.MinValue)
            {
                CreatureDeadline.Text = string.Format("永久");
            }
            else
            {
                var seconds = (GameScene.User.IntelligentCreatures[selectedCreature].Expire - CMain.Now).TotalSeconds;

                CreatureDeadline.Text = string.Format("到期：{0}", Functions.PrintTimeSpanFromSeconds(seconds));
            }
    ...
    {
        public readonly string[] OptionNames = { "全部", "金币", "武器", "盔甲", "头盔", "靴子", "腰带", "首饰", "其他" };
    ...
    {
        private string[] GradeStrings = { "全部", "普通", "稀有", "史诗", "传说", "英雄" };
