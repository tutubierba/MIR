...
>>>>>>> 9a77f39 (副本功能添加)
    ...
    public sealed class NPCGoodsDialog : MirImageControl
    {
        ...
        private void BuyItem()
        {
            ...
            if (SelectedItem.Info.StackSize > 1)
            {
                ...
                if (UsePearls)
                {
                    if (SelectedItem.Price() > GameScene.User.PearlCount)
                    {
                        ...
                        if (maxQuantity == 0)
                        {
                            GameScene.Scene.ChatDialog.ReceiveChat("你没有足够的积分.", ChatType.System);
                            return;
                        }
                    }
                }
            ...
            else
            {
                ...
                {
                    GameScene.Scene.ChatDialog.ReceiveChat("太重了，你已经背不动了.", ChatType.System);
                    return;
                }
                ...
                for (int i = 0; i < MapObject.User.Inventory.Length; i++)
                {
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat("你不能购买更多的物品了.", ChatType.System);
                        return;
                    }
                }
        ...
        private void Confirm()
        {
            ...
            switch (PType)
            {
                case PanelType.Sell:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat("无法出售此物品.", ChatType.System);
                        return;
                    }
                ...
                case PanelType.Repair:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat("无法修理这个物品.", ChatType.System);
                        return;
                    }
                ...
                case PanelType.Consign:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat("无法寄售这个物品.", ChatType.System);
                        return;
                    }
                ...
                case PanelType.Refine:
                    ...
                    for (int i = 0; i < GameScene.Scene.RefineDialog.Grid.Length; i++)
                    {
                        if (GameScene.Scene.RefineDialog.Grid[i].Item != null)
                        {
                            ...
                            GameScene.Scene.ChatDialog.ReceiveChat(String.Format("你没有足够的金币精炼{0}.", TargetItem.FriendlyName), ChatType.System);
                            return;
                        }

                    }
                    GameScene.Scene.ChatDialog.ReceiveChat(String.Format("你没有放入任何材料精炼{0}.", TargetItem.FriendlyName), ChatType.System);
                    break;
                case PanelType.CheckRefine:

                    if (TargetItem.RefineAdded == 0)
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(String.Format("你的{0}尚未精炼，无需检查.", TargetItem.FriendlyName), ChatType.System);
                        return;
                    }
    ...
    public sealed class NPCAwakeDialog : MirImageControl
    {
        ...
        public void ItemCell_Click()
        {
            ...
            if (Items[0] == null)
            {
                SelectAwakeType.Items.Add("放入觉醒装备.");
            ...
            else
            {
                ...
                {
                    SelectAwakeType.Items.Add("选择觉醒雕文.");
                    if (Items[0].Info.Type == ItemType.Weapon)
                    {
                        SelectAwakeType.Items.Add("勇气雕文");
                        SelectAwakeType.Items.Add("魔法雕文");
                        SelectAwakeType.Items.Add("灵魂雕文");
                    }
                    else if (Items[0].Info.Type == ItemType.Helmet)
                    {
                        SelectAwakeType.Items.Add("防护雕文");
                        SelectAwakeType.Items.Add("守护雕文");
                    }
                    else
                    {
                        SelectAwakeType.Items.Add("生机雕文");
                    }
                }
                else
                {
    ...
    public sealed class CraftDialog : MirImageControl
    {
        ...
        private void CraftItem()
        {
            ...
            if (max > 1)
            {
                MirAmountBox amountBox = new MirAmountBox("制作数量:", RecipeItem.Info.Image, max, 0, max);
                ...
                amountBox.OKButton.Click += (o, e) =>
                {
                    if (amountBox.Amount > 0)
                    {
                        ...
                        {
                            GameScene.Scene.ChatDialog.ReceiveChat("所需材料不足.", ChatType.System);
                            return;
                        }
        ...
        public void RefreshCraftCells(UserItem selectedItem)
        {
            ...
            PossibilityLabel.Text = (UserObject.User.Stats[Stat.CraftRatePercent] > 0 ? $"{Math.Min(100, Recipe.Chance + UserObject.User.Stats[Stat.CraftRatePercent])}% (+{UserObject.User.Stats[Stat.CraftRatePercent]}%)" : $"{Recipe.Chance}%") + "成功率";
    ...
    public sealed class StorageDialog : MirImageControl
    {
        ...
        public StorageDialog()
        {
            ...
            RentButton.Click += (o, e) =>
            {
                ...
                if (GameScene.User.HasExpandedStorage)
                    messageBox = new MirMessageBox("你想花费1000000金币以延长10天扩展仓库租赁期吗？", MirMessageBoxButtons.OKCancel);
                else
                    messageBox = new MirMessageBox("你想花费1000000金币租赁10天额外的仓库存储吗？", MirMessageBoxButtons.OKCancel);
        ...
        public void RefreshStorage2()
        {
            ...
            if (GameScene.User.HasExpandedStorage)
            {
                ...
                LockedPage.Visible = false;
                RentalLabel.Text = "扩展仓库到期：" + GameScene.User.ExpandedStorageExpiryTime.ToString();
                RentalLabel.ForeColour = Color.White;
            }
            else
            {
                RentalLabel.Text = "扩展仓库已锁定";
