...
namespace Client.MirScenes.Dialogs
{
    public sealed class TrustMerchantDialog : MirImageControl
    {
        ...
        private readonly string consignmentText = $"1.寄售每件物品收取{Globals.ConsignmentCost}金币 \r\n\r\n2.交易完成收取寄售价格1%的手续费\r\n\r\n" +
            $"3.最长寄售时间：{Globals.ConsignmentLength}天\r\n\r\n4.最大寄售数量不限\r\n\r\n" +
            $"5.寄售价格限制：{Globals.MinConsignment} - {Globals.MaxConsignment}金币";

        private readonly string auctionText = $"1.每件物品拍卖费用为：{Globals.AuctionCost}金币，最高起拍价为：{Globals.MaxStartingBid}金币\r\n\r\n2.收取拍卖成交价格1%的手续费\r\n\r\n" +
            $"3.最长拍卖时间：{Globals.ConsignmentLength}天，最终最高出价者得标\r\n\r\n4.最大拍卖数量不限\r\n\r\n";
        ...
        public TrustMerchantDialog()
        {
            ...
            MailButton.Click += (o, e) =>
            {
                ...
                string message = $"出价{Selected.Listing.Item.FriendlyName}购买{Selected.Listing.Price}.";
            ...
            BuyButton.Click += (o, e) =>
            {
                ...
                if (UserMode)
                {
                    if (Selected.Listing.ItemType == MarketItemType.Consign)
                    {
                        ...
                        {
                            MirMessageBox box = new MirMessageBox(string.Format("{0}尚未售出，确定要取回吗？", Selected.Listing.Item.FriendlyName), MirMessageBoxButtons.YesNo);
                    ...
                    {
                        if (Selected.Listing.Seller == "未出价")
                        {
                            MirMessageBox box = new MirMessageBox(string.Format("{0尚未售出，确定要取回吗？", Selected.Listing.Item.FriendlyName), MirMessageBoxButtons.YesNo);
                            box.YesButton.Click += (o1, e2) =>
                            {
                                MarketTime = CMain.Time + 3000;
                                Network.Enqueue(new C.MarketGetBack { AuctionID = Selected.Listing.AuctionID });
                ...
                else
                {
                    switch (Selected.Listing.ItemType)
                    {
                        ...
                        case MarketItemType.GameShop:
                            {
                                MirMessageBox box = new MirMessageBox(string.Format("你确定要花费{0:#,##0} {1}购买{2}吗？", Selected.Listing.Item.FriendlyName, Selected.Listing.Price, MarketType == MarketPanelType.GameShop ? "Credits" : "Gold"), MirMessageBoxButtons.YesNo);
                                box.YesButton.Click += (o1, e2) =>
                                {
                                    MarketTime = CMain.Time + 3000;
                        ...
                        case MarketItemType.Auction:
                            {
                                MirAmountBox bidAmount = new MirAmountBox("竞价金额:", Selected.Listing.Item.Info.Image, uint.MaxValue, Selected.Listing.Price + 1, Selected.Listing.Price + 1);

                                bidAmount.OKButton.Click += (o1, e1) =>
                                {
                                    MirMessageBox box = new MirMessageBox(string.Format("你确定要以{0:#,##0}金币竞标{1}吗？", bidAmount.Amount, Selected.Listing.Item.FriendlyName), MirMessageBoxButtons.YesNo);
        ...
        private void SetupFilters()
        {
            ...
            var drapery = new Filter { Index = 2, Title = "装饰", Type = null };
            var accessory = new Filter { Index = 3, Title = "辅助", Type = null };
            var consumable = new Filter { Index = 4, Title = "消耗", Type = null };
            var enhancement = new Filter { Index = 5, Title = "强化", Type = null };
            var book = new Filter { Index = 6, Title = "书籍", Type = null };
            var crafting = new Filter { Index = 7, Title = "工艺", Type = null };
            ...
            drapery.SubFilters.Add(new Filter { Index = 201, Title = "盔甲", Type = ItemType.Armour });
            ...
            drapery.SubFilters.Add(new Filter { Index = 205, Title = "材料", Type = ItemType.Stone });
            ...
            consumable.SubFilters.Add(new Filter { Index = 401, Title = "恢复药剂", Type = ItemType.Potion, MaxShape = 2 });
            consumable.SubFilters.Add(new Filter { Index = 402, Title = "增益药剂", Type = ItemType.Potion, MinShape = 3, MaxShape = 4 });
            consumable.SubFilters.Add(new Filter { Index = 403, Title = "卷轴/油", Type = ItemType.Scroll });
            consumable.SubFilters.Add(new Filter { Index = 404, Title = "其他", Type = ItemType.Script });

            enhancement.SubFilters.Add(new Filter { Index = 501, Title = "宝石", Type = ItemType.Potion, MinShape = 3, MaxShape = 3 });
            enhancement.SubFilters.Add(new Filter { Index = 502, Title = "矿石", Type = ItemType.Potion, MinShape = 4, MaxShape = 4 });
            ...
            crafting.SubFilters.Add(new Filter { Index = 701, Title = "布料", Type = ItemType.CraftingMaterial });
        ...
        public void TMerchantDialog(MarketPanelType type)
        {
            ...
            switch (type)
            {
                case MarketPanelType.Market:
                    ...
                    TitleSalePriceLabel.Text = "出售价格";
                    TitleSellLabel.Text = "出售物品";
                    TitleItemLabel.Text = "出售";
                    TitlePriceLabel.Text = "价格/竞标";
                    TitleExpiryLabel.Text = "出售人/到期";
                ...
                case MarketPanelType.Auction:
                    ...
                    TitlePriceLabel.Visible = true;
                    TitleExpiryLabel.Visible = true;
                    TitleSalePriceLabel.Text = "起拍价";
                    TitleSellLabel.Text = "出售物品";
                    TitleItemLabel.Text = "物品";
                    TitlePriceLabel.Text = "最高出价";
