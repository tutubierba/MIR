...
namespace Client.MirScenes
{
    public class SelectScene : MirScene
    {
        ...
        private void NewCharacter(S.NewCharacter p)
        {
            ...
            switch (p.Result)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    MirMessageBox.Show("性别不存在\n请联系管理员.");
                    break;
                case 3:
                    MirMessageBox.Show("职业未开放\n请联系管理员.");
                    break;
                case 4:
                    MirMessageBox.Show("不能创建超过 " + Globals.MaxCharacterCount + "个角色.");
                    _character.Dispose();
                    break;
                case 5:
                    MirMessageBox.Show("角色名的已存在.");
                    _character.NameTextBox.SetFocus();
                    break;
            }
        }
        ...
        private void DeleteCharacter()
        {
            ...
            MirMessageBox message = new MirMessageBox(string.Format("你确定要删除{0}吗？?", Characters[_selected].Name), MirMessageBoxButtons.YesNo);
            int index = Characters[_selected].Index;

            message.YesButton.Click += (o1, e1) =>
            {
                MirInputBox inputBox = new MirInputBox("请输入角色名称.");
        ...
        private void DeleteCharacter(S.DeleteCharacter p)
        {
            ...
            {
                case 0:
                    MirMessageBox.Show("禁止删除角色.");
                    break;
                case 1:
                    MirMessageBox.Show("角色不存在\n请联系管理员.");
                    break;
            }
        }
        ...
        private void StartGame(S.StartGameDelay p)
        {
            ...
            MirMessageBox message = new MirMessageBox(string.Format("{0}秒内无法登陆.", Math.Ceiling(p.Milliseconds / 1000M)));

            message.BeforeDraw += (o, e) => message.Label.Text = string.Format("{0}秒内无法登陆.", Math.Ceiling((time - CMain.Time) / 1000M));
        ...
        public void StartGame(S.StartGameBanned p)
        {
            ...
            MirMessageBox.Show(string.Format("账号被封禁\n\n原因：{0}\n解封日期：{1}\n持续：{2:#,##0}小时，{3}分钟，{4}秒", p.Reason,
        ...
        public void StartGame(S.StartGame p)
        {
            ...
            {
                case 0:
                    MirMessageBox.Show("禁止进入游戏.");
                    break;
                case 1:
                    MirMessageBox.Show("未登录.");
                    break;
                case 2:
                    MirMessageBox.Show("角色未找到.");
                    break;
                case 3:
                    MirMessageBox.Show("未找到初始地图或出生点.");
                    break;
                case 4:
