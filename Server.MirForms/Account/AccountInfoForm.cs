...
namespace Server
{
    public partial class AccountInfoForm : Form
    {
        ...
        {
            if (MessageBox.Show("确定要封禁所选账号?", "Ban Selected.", MessageBoxButtons.YesNoCancel) != DialogResult.Yes) return;
        ...
        private void WipeCharButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("服务器运行时无法删除数据", "Notice",
                MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return;
            }

            if (MessageBox.Show("确定要清除所有数据?", "Notice",
                 MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
            {
                ...
                MessageBox.Show("已清除所有相关数据", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
