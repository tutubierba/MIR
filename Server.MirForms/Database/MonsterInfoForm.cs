...
namespace Server
{
    public partial class MonsterInfoForm : Form
    {
        ...
        MLibraryV2 _testmy;
        MLibraryV2.MImage _selectImage;
        string fileName = "";
        int aa = -1;
        Thread th = null;

        public string BINT(int a)
        {
            if (a < 10)
            {
                return "00" + a + ".Lib";
        ...
        private void ShowImgs()
        {
            ...
            while (ShowImage)
            {
                if (aa != -1)
                {
                    ...
                    while (x)
                    {
                        for (int i = 0; i < _testmy.Images.Count; i++)
                        {
                            if (loca != aa)
                            {
                                x = false;
                            ...
                            Thread.Sleep(200);
                            _selectImage = _testmy.GetMImage(i);
                            if (_selectImage != null)
                            {
                                ImageBox1.Image = _selectImage.Image;
                            }
                        }
                    }
                }
        ...
        private void RemoveButton_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("你确定要删除选定的怪物吗?", "删除怪物?", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
