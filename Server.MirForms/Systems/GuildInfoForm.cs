...
namespace Server
{
    public partial class GuildInfoForm : Form
    {
        ...
        {
            if (ActiveControl != sender) return;
            if (MessageBox.Show("是否确实要删除最后一个索引?", "删除?", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
        ...
        private void BuffDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("是否确实要删除选定的行会增益?", "删除增益?", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
