...
namespace LibraryEditor
{
    public partial class LMain : Form
    {
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("是否确实要删除所选图像?",
                "删除选定项.",
        ...
        {
            if (MessageBox.Show("是否确实要删除空白图像?",
                "删除空白",
        ...
        private void ExportButton_Click(object sender, EventArgs e)
        {
            ...
            toolStripProgressBar.Value = 0;
            MessageBox.Show("正在保存到 " + _folder + "...", "图像已保存", MessageBoxButtons.OK);
        }
        ...
        private void autofillNpcFramesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show($"是否确实要填充 {files.Count()} Libs with their matching FrameSet?",
                "自动填充.",
                MessageBoxButtons.YesNo) != DialogResult.Yes) return;
        ...
        private void UpdateFrameGridData()
        {
            ...
            foreach (DataGridViewRow row in frameGridView.Rows)
            {
                ...
                {
                    MessageBox.Show(string.Format($"行动 '{action}' 存在多次，因此不会保存."));
                    continue;
                }
